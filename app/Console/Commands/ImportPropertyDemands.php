...
class ImportPropertyDemands extends Command
{
    ...
    {
        $url = rtrim(config('constants.oldAllDemandByPropertyId'), '/') . '?PropertyID=' . urlencode($propertyId);
        ...
        $data          = $json['Data'] ?? [];
        $allDemands    = $data['AllDemanddetails'] ?? [];     // <-- changed
        $subheads      = $data['SubHeadwiseBreakup'] ?? [];

        if (empty($allDemands)) {
            Log::info("Property {$propertyId} has no AllDemanddetails.");
            return;
        }

        foreach ($allDemands as $demandRow) { 
            ...
            //added - for all demand on 23122025 by Swati

            $alreadyImported = OldDemand::where('demand_id', $demandId)
                ->where('property_id', $propIdFromApi)
                ->where('status', 1)
                ->exists();

            if ($alreadyImported) {
                Log::info("Skipping demand {$demandId} for property {$propIdFromApi} (already imported).");
                continue;
            }
    ...
    {
        //for fetching single property demand
        $masterProperty = 40117;
        $propertyId = 21404;
        $this->handleDemand($masterProperty, $propertyId);


        //for all properties
        // $importedDemandsCount = OldDemand::where('status', 1)->count();
        // $this->info("Already imported demands (old_demands.status = 1): {$importedDemandsCount}");
        ...
        // SplitedPropertyDetail::orderBy('id')
