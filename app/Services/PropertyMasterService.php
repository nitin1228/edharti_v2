...
class PropertyMasterService
{
    ...
    public function checkPropertyIsInUserSectoin($propertyId)
    {
        ...
        if (in_array($user->roles[0]->name, ['section-officer', 'deputy-lndo','assistant-section-officer','superintendent'])) {
            $userSectionCodes = $user->sections->pluck('section_code')->toArray();
            $hasAccess = PropertyMaster::where('id',$propertyId)->whereIn('section_code', $userSectionCodes)->exists();

            //commented as we are taking sections from property masters not mapping
    ...
    {
        // dd(Auth::user()->sections->pluck('section_code')->toArray());
        $userSectionIds = Auth::user()->sections->pluck('section_code')->toArray();
        return DB::table('property_masters as pm')
            ->leftJoin('splited_property_details as spd', 'pm.id', 'spd.property_master_id')
            ->whereIn('pm.section_code',$userSectionIds)
    ...
    public function conversionCharges($propertyId, $remission = 0, $surcharge = 0, $chargesOnly = true, $date = null)
    {
        ...
        if ($propertyId != "") {
            ...
            // Config::get();
            $landRateService = new LandRateService();
            $landRateRow = $landRateService->getLandRates($landRateType, $propertyTypeName, $colonyId, $date);
            if (isset($landRateRow['error'])) {
                return response()->json(['status' => 'error', 'details' => config('messages.landUseChange.error.dataNotAvailable')]);
            }
            if (empty($landRateRow)) {
