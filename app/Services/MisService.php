...
class MisService
{
    ...
    {
        if(isset($request->transferred) && $request->transferred ==1) {
        ...
        if (isset($request->landTransferTypeNew)) {
			...
			PropertyTransferredLesseeDetail::where('property_master_id', $id)->where('process_of_transfer',"!=","Conversion")->where('batch_transfer_id', '!=', 1)->whereNull('splited_property_detail_id')->delete();
		}
    }
    ...
    {
        //dd($request->all());
        if(isset($request->freeHold) && $request->freeHold =="Yes") {
            ...
            //   $isPrevBatchId = PropertyTransferredLesseeDetail::withTrashed()->where('property_master_id', $id)->max('batch_transfer_id');
            $isPrevBatchId = PropertyTransferredLesseeDetail::where('property_master_id', $id)->max('batch_transfer_id');
            if ($isPrevBatchId > 0) {
                $batch_transfer_id = $isPrevBatchId + 1;
                $previous_batch_transfer_id = $isPrevBatchId;
            } else {
            ...
            PropertyTransferredLesseeDetail::where('property_master_id', $id)->where('process_of_transfer',"Conversion")->delete();
    ...
    public function updateMiscellaneousDetail($id, $request, $old_property_id)
    {
        ...
        // updated by Swati Mishra on 10032026 as when re_entred, gr_revised and supplimentry_lease_deed_excecuted is set to 0 from 1 dates need to be removed
        $grRevised = (int) $request->GR;
        $supplementaryExecuted = (int) $request->Supplementary;
        $isReRented = (int) $request->Reentered;

        
        //Normalize GR revised flag/date
        
        if ($grRevised === 1) {
            if (empty($request->gr_revised_date)) {
                throw ValidationException::withMessages([
                    'gr_revised_date' => 'GR revised date is required as property ground rent is revised.'
                ]);
            }
            $grRevisedDate = $request->gr_revised_date;
        } else {
            $grRevisedDate = null;
        }

        
        //Normalize supplementary lease deed flag/date
        
        if ($supplementaryExecuted === 1) {
            if (empty($request->supplementary_date)) {
                throw ValidationException::withMessages([
                    'supplementary_date' => 'Supplementary lease deed date is required as supplementary lease deed is executed.'
                ]);
            }
            $supplementaryExecutedDate = $request->supplementary_date;
        } else {
            $supplementaryExecutedDate = null;
        }

        //Normalize re-rented flag/date
        
        if ($isReRented === 1) {
            if (empty($request->date_of_reentry)) {
                throw ValidationException::withMessages([
                    'date_of_reentry' => 'Re-entered date is required as property is re-entered.'
                ]);
            }
            $reRentedDate = $request->date_of_reentry;
        } else {
            $reRentedDate = null;
        }
        ...
        if (isset($propertyMiscDetail)) {
            ...
            // updated by Swati Mishra on 10032026 as when gr_revised and supplimentry_lease_deed_excecuted is set to 0 from 1 dates need to be removed

            $propertyMiscDetail->is_gr_revised_ever = $grRevised;
            $propertyMiscDetail->gr_revised_date = $grRevisedDate;
            $propertyMiscDetail->is_supplimentry_lease_deed_executed = $supplementaryExecuted;
            $propertyMiscDetail->supplimentry_lease_deed_executed_date = $supplementaryExecutedDate;
            ...
            // $propertyMiscDetail->re_rented_date = $request->date_of_reentry;
             ...
             // updated by Swati Mishra on 10032026 as when re_entered is set to 0 from 1 date need to be removed
            $propertyMiscDetail->is_re_rented = $isReRented;
            $propertyMiscDetail->re_rented_date = $reRentedDate;
        ...
        try {
            ...
            if ($iseditedOrApprovedEver) {
                if ($iseditedOrApprovedEver->is_active == 1 && $iseditedOrApprovedEver->permission_to == Auth::user()->id) {
                    ...
                    $checkApplicationRecExists = ApplicationStatus::where([['service_type', $serviceType],['model_id', $modalId]])->latest()->first();
                    if ($checkApplicationRecExists) {
                        ...
                        // dd($checkApplicationRecExists);
                } 
            } else {
                $applicantNo = $request->applicantNo;
                $applicationStatus = ApplicationStatus::where('service_type', $serviceType)->where('model_id', $modalId)->latest()->first();
