...
use App\Models\ItCellRegTransferHistory;
use Carbon\Carbon;
...
class OfficialController extends Controller
{
    ...
    public function getRegisteredUsers(Request $request)
    {
       ...
       $roleName = $user->roles->pluck('name')->first();
        ...
        $query = UserRegistration::query()
            ...
            ->leftJoin('items as land_type_items', 'user_registrations.land_type', '=', 'land_type_items.id') // Add this line for land_type
            ...
            ->select(
                ...
                'items.item_name',
                'items.item_code',
                'land_type_items.item_name as land_type_name', // Add this line
        ...
        $columns = ['id', 'applicant_number', 'name','land_type_name', 'property_details','flat_number', 'user_type','purpose_of_registation','section','activity','status', 'remarks', 'created_at'];
        ...
        if (!empty($request->input('search.value'))) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('user_registrations.applicant_number', 'LIKE', "%{$search}%")
                    ...
                    ->orWhere('user_registrations.email', 'LIKE', "%{$search}%")
                    ...
                    ->orWhere('land_type_items.item_name', 'LIKE', "%{$search}%") // Add this line to search by land type name
        ...
        $assignedBySection = $earliestBySection = []; // code modified by swati on 17022026
        $fifoMode = 'global'; // code modified by swati on 17022026
        ...
        if ($totalFiltered > 0) {
            ...
            $assignedBySection = $assignedToUSerData['all_assigned_section'] ?? []; // code modified by swati on 17022026
            $earliestBySection = $assignedToUSerData['earliest_section'] ?? []; // code modified by swati on 17022026
            $fifoMode = $assignedToUSerData['mode'] ?? 'global'; // code modified by swati on 17022026
        }
        ...
        foreach ($getRegistrationDetails as $key => $getRegistrationDetail) {
            ...
            if ($getRegistrationDetail->forward_through == 'cron' && $getRegistrationDetail->action_taken_by == 'deputy-lndo') {
                $autoForward = '(Auto Forward) <div class="alertGreen"></div>';
            }
            $actionBtnHtml = '';
           ...
           // $nestedData['applicant_number'] = $getRegistrationDetail->applicant_number;
            ...
            if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertRed"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
            } else {
                $nestedData['applicant_number'] = $getRegistrationDetail->applicant_number;
            } 
           $nestedData['name'] = $getRegistrationDetail->name;
           $nestedData['email'] = $getRegistrationDetail->email; // Add this line
            $nestedData['land_type_name'] = $getRegistrationDetail->land_type_name;
            if (!Is_null($assignedToUSer)) {
                            $appNo = $getRegistrationDetail->applicant_number; // code modified by swati on 17022026
            $secId = $getRegistrationDetail->section_id; // code modified by swati on 17022026

            $useSectionWiseHere = ($fifoMode === 'by_section') // code modified by swati on 17022026
                && !empty($earliestBySection) // code modified by swati on 17022026
                && !empty($assignedBySection) // code modified by swati on 17022026
                && !empty($secId); // code modified by swati on 17022026

            if ($useSectionWiseHere) { // code modified by swati on 17022026
                $earliestList = $earliestBySection[$secId] ?? []; // code modified by swati on 17022026
                $assignedList = $assignedBySection[$secId] ?? []; // code modified by swati on 17022026

                if ((in_array($appNo, $earliestList))) // code modified by swati on 17022026
                {
                    if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                        $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $appNo . '<div class="alertRed"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                    } else {
                        if ($getRegistrationDetail->action_taken_by == 'section-officer') {
                            $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $appNo . '<div class="alertGreen"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                        } else {
                            $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $appNo . '</div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                        }
                    }
                } else if (
                    in_array($appNo, $assignedList) // code modified by swati on 17022026
                ) {
                    if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                        $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $appNo . '<div class="alertRed"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                    } else {
                        $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $appNo . '<div class="alertDot"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                    }
                }
             } else {
                    ...
                    if ((in_array($getRegistrationDetail->applicant_number, $earliest))) // code modfied by nitin on 03-09-2025
                    {
                        if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                            ...
                            if ($getRegistrationDetail->action_taken_by == 'section-officer') {
                                $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertGreen"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                            } else {
                                $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '</div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                            }
                        }
                    } else if (
                        ...
                        if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                            ...
                            $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertDot"></div></div><div style="color:#116d6e;" class="d-flex gap-1 align-items-center">' . $autoForward . '</div>';
                        }
                    }
                }
            }
           if (is_null($getRegistrationDetail->locality)) {
            ...
            $nestedData['updated_at'] = Carbon::parse($getRegistrationDetail->updated_at)->format('d-m-Y H:i:s');
        ...
        try {
            DB::transaction(function () use ($id, $request) {
                ...
                if ($user) {
                    ...
                    // soft delete user properties (unlink property)
                    UserProperty::where('user_id', $user->id)->delete();
         ...
         /* private function getAssignedToAuthUser()
    {
        ...
        $assignedApplications = collect(); // to collect all assigned
        $filfoLimit = $user->roles[0]->name == 'section-officer' ? 10 : 3;
        if ($user->roles[0]->name == 'section-officer') {
            // 1. New applications in section
            $newApplications = UserRegistration::whereIn('section_id', $user->sections->pluck('id')->toArray())
                ->where('status', getServiceType('RS_NEW'))
        ...
        private function getAssignedToAuthUser()
    {
        $user = Auth::user();
           ...
           $fifoLimit = 10; // keep same as current behavior
        $sectionIds = $user->sections->pluck('id')->toArray();

        // Enable section-wise FIFO only when user has more than 1 section (your requirement) // code modified by swati on 17022026
        $useSectionWise = count($sectionIds) > 1; // code modified by swati on 17022026

        $assignedRows = collect(); // unified rows: application_no, created_at, section_id

        // 1) SECTION-OFFICER: include "new applications in section" (existing logic)      
        if ($user->roles[0]->name == 'section-officer') {
            // 1. New applications in section
        $newApplications = UserRegistration::whereIn('section_id', $sectionIds)
                    ->where('status', getServiceType('RS_NEW'))
                    ->select('applicant_number', 'created_at', 'section_id')
                    ->get()
                    ->map(fn($r) => [
                        'application_no' => $r->applicant_number,
                        'created_at'     => $r->created_at,
                        'section_id'     => $r->section_id,
                    ]);

                $assignedRows = $assignedRows->merge($newApplications);
            }

            // 2) ALL ROLES: latest movements assigned to user (existing logic)
            $latestMovements = ApplicationMovement::select('application_no', 'created_at')
                ->where('service_type', getServiceType('RS_NEW_REG'))
                ->whereIn('id', function ($query) {
                    $query->selectRaw('MAX(id)')
                        ->from('application_movements')
        ...
        $movementAppNos = $latestMovements->pluck('application_no')->unique()->values()->toArray();

        // Map application_no -> section_id from user_registrations // code modified by swati on 17022026
        // (Needed only for section-wise FIFO; safe to compute always) // code modified by swati on 17022026
        $sectionMap = []; // code modified by swati on 17022026
        if (!empty($movementAppNos)) {
            $sectionMap = UserRegistration::whereIn('applicant_number', $movementAppNos)
                ->select('applicant_number', 'section_id')
                ->pluck('section_id', 'applicant_number')
                ->toArray();
        }

        $movementRows = $latestMovements->map(function ($m) use ($sectionMap) { // code modified by swati on 17022026
            $secId = $sectionMap[$m->application_no] ?? null; // code modified by swati on 17022026
            return [
                'application_no' => $m->application_no,
                'created_at'     => $m->created_at,
                'section_id'     => $secId,
            ];
        })->filter(fn($x) => !is_null($x['section_id'])); // code modified by swati on 17022026

        $assignedRows = $assignedRows->merge($movementRows); // code modified by swati on 17022026

        // Safety: keep only user's sections (prevents cross-section bleed) // code modified by swati on 17022026
        if (!empty($sectionIds)) {
            $assignedRows = $assignedRows->whereIn('section_id', $sectionIds)->values(); // code modified by swati on 17022026
        }

        // Deduplicate by section + application_no (so per-section FIFO is consistent) // code modified by swati on 17022026
        $assignedRows = $assignedRows
            ->unique(fn($x) => $x['section_id'] . '|' . $x['application_no']) // code modified by swati on 17022026
            ->values();

        // ---- Existing GLOBAL FIFO outputs (do not change behavior) ----
        $globalSorted = $assignedRows->sortBy('created_at')->values();

        $allFlat = $globalSorted->pluck('application_no')->unique()->values()->toArray();
        $earliestFlat = $globalSorted->take($fifoLimit)->pluck('application_no')->toArray();

        // ---- New SECTION-WISE FIFO outputs (only used when >1 section) ---- // code modified by swati on 17022026
        $grouped = $assignedRows
            ->groupBy('section_id') // code modified by swati on 17022026
            ->map(fn($rows) => $rows->sortBy('created_at')->values()); // code modified by swati on 17022026

        $allBySection = $grouped
            ->map(fn($rows) => $rows->pluck('application_no')->toArray()) // code modified by swati on 17022026
            ->toArray();

        $earliestBySection = $grouped
            ->map(fn($rows) => $rows->take($fifoLimit)->pluck('application_no')->toArray()) // code modified by swati on 17022026
            ->toArray();

        return [
            // Keep old keys EXACTLY (backward compatible)
            'all_assigned' => $allFlat,
            'earliest'     => $earliestFlat,

            // New keys (optional usage) // code modified by swati on 17022026
            'all_assigned_section' => $allBySection, // code modified by swati on 17022026
            'earliest_section'     => $earliestBySection, // code modified by swati on 17022026
            'mode'                 => $useSectionWise ? 'by_section' : 'global', // code modified by swati on 17022026
            'fifo_limit'           => $fifoLimit,
        ];
    }
            ...
            if (!empty($id)) {
               ...
               $assignedToUSer = $assignedToUSerData['all_assigned'];
                ...
                if (in_array($applicantNumber, $assignedToUSer)) {
                    ...
                    // Section-wise FIFO validation when enabled // code modified by swati on 17022026
                    if ($fifoMode === 'by_section' && !empty($earliestBySection) && !empty($assignedBySection) && !empty($regUserDetails->section_id)) { // code modified by swati on 17022026

                        $secId = $regUserDetails->section_id; // code modified by swati on 17022026
                        $assignedList = $assignedBySection[$secId] ?? []; // code modified by swati on 17022026
                        $earliestList = $earliestBySection[$secId] ?? []; // code modified by swati on 17022026

                        // Only enforce FIFO if this application is part of this section's assigned list // code modified by swati on 17022026
                        if (in_array($applicantNumber, $assignedList) && !(in_array($applicantNumber, $earliestList))) { // code modified by swati on 17022026
                            return redirect()->back()->with(
                                'failure',
                                'Older registration' . (count($earliestList) > 1 ? 's ' : ' ') . implode(', ', $earliestList) .
                                ' is pending, please process ' . (count($earliestList) > 1 ? 'it ' : 'them ') . ' first.'
                            ); // code modified by swati on 17022026
                        }

                    } 
                    else {
                        // Fallback: existing global FIFO validation (unchanged) // code modified by swati on 17022026
                ...
                if ($regUserDetails) {
                    ...
                    if(!is_null($regUserDetails->generated_pid) && isset($regUserDetails->generated_pid)){
                        //Fetch Suggested Property Id from Property Master
                        $property = PropertyMaster::where('old_propert_id', $regUserDetails->generated_pid)->latest('created_at')->first();
                    } else if (!is_null($regUserDetails->correct_property_id) && isset($regUserDetails->correct_property_id)) {
                        //Fetch Suggested Property Id from Property Master
                        $property = PropertyMaster::where('old_propert_id', $regUserDetails->correct_property_id)->latest('created_at')->first();
                        if (empty($property)) {
                            $splitedProperty = SplitedPropertyDetail::where('old_property_id', $regUserDetails->correct_property_id)->first();
                            if (!empty($splitedProperty)) {
                                $isSplited = 1;
                                $splited_primary_id = $splitedProperty->id;
                                $data['splitedPropertyId'] = $splited_primary_id;
                                $property = PropertyMaster::find($splitedProperty->property_master_id);
                            }
                        }
                    } else {
                        $property = PropertyMaster::where('new_colony_name', $regUserDetails->locality)->where('block_no', $regUserDetails->block)->where('plot_or_property_no', $regUserDetails->plot)->first();
                        if(empty($property))
                        {
                            $properties = PropertyMaster::where('new_colony_name', $regUserDetails->locality)->where('block_no', $regUserDetails->block)->where('is_joint_property', 1)->pluck('id')->toArray();

                            $splitedProperty = SplitedPropertyDetail::whereIn('property_master_id', $properties)->where('plot_flat_no',$regUserDetails->plot)->first();
                            if(!empty($splitedProperty))
                            {
                                $isSplited = 1;
                                $splited_primary_id = $splitedProperty->id;
                    ...
                    if ($property && !empty($property->id)) {
                     if ($property->is_joint_property) {
                        ...
                        // dd($scanningLink . ($splitedProperty ? $splitedProperty->old_property_id:$property['old_propert_id']));
                        $response = Http::timeout(10)->get( $scanningLink . ($splitedProperty ? $splitedProperty->old_property_id:$property['old_propert_id']));
                        ...
                        if (isset($response) && $response->successful()) {
                            ...
                            if (!empty($jsonData)) {
                               ...
                               foreach ($jsonData[0]['ListFileName'] as $item) {
                                    $scannedFiles['files'][] = [
                                        'encrypted' => $item['PropertyFilepdf'],
                                        'file_name' => $item['PropertyFileNameDcrpt'],
                                    ];
                                }
                                                                // dd($scannedFiles['files']);
                        ...
                        $splittedId = $splitedProperty ? $splitedProperty->id : null;
                        $scannedFiles = GeneralFunctions::getLocalScannedFilesNew($propertyId, $splittedId);
                        $data['propertyMasterId'] = $property['id'];
                        //  dd($splitedProperty);
                        // $data['isSplited'] = $isSplited;
                        // $data['suggestedPropertyId'] = $property['old_propert_id'];
                        $data['suggestedPropertyId'] = $splitedProperty ? $splitedProperty->old_property_id:$property['old_propert_id'];
                        $data['oldPropertyId'] = $splitedProperty ? $splitedProperty->old_property_id:$property['old_propert_id'];
                    ...
                    $regUserDetails->landType = getServiceNameById($regUserDetails->land_type);
                   ...
                   if ($data['details']->status != getStatusName('RS_UREW')) {
                    ...
                    // dd($data);
                    return view('officials.register-users.details', compact(['data', 'roles', 'checkList', 'scannedFiles', 'isApproved', 'isSplited', 'splited_primary_id']));
//                     return view('officials.register-users.details', compact(['data', 'roles', 'checkList', 'scannedFiles']));
                        ...
                        if (isset($response) && $response->successful()) {
                            ...
                            if (!empty($jsonData)) {
                                ...
                                foreach ($jsonData[0]['ListFileName'] as $data) {
                                    $scannedFiles['files'][] = $data['PropertyFileNameDcrpt'];
                                }
                            } else {
                                // Handle case where the response is empty or not as expected
                                Log::warning('API response returned empty or invalid data.');
    ...
    public function getApplicantPropertyListings(Request $request)
    {
        ...
        $query = NewlyAddedProperty::query()
            ...
            ->leftJoin('items as land_type_items', 'newly_added_properties.land_type', '=', 'land_type_items.id') // Add this line for land_type
        ...
        $columns = ['id', 'applicant_number', 'name','land_type_name', 'property_details', 'remarks', 'status', 'created_at'];
        ...
        foreach ($getNewPropertyDetails as $key => $getNewPropertyDetail) {
            ...
            $nestedData['land_type_name'] = $getNewPropertyDetail->land_type_name;
    ...
    public function newPropertyDetails($id)
    {
        if (!empty($id)) {
            ...
            // dd($id,$regUserDetails);
            $data['splitedPropertyId'] = 0;
            $data['isSplited'] = 0;
            $scannedFiles = [];
            // dd($regUserDetails);
            if ($regUserDetails) {
                $splitedProperty = null;
                $oldPropertyId = null;
                $suggestedPropertyId = null;

                if (!is_null($regUserDetails->correct_property_id) && isset($regUserDetails->correct_property_id)) {
                    //Fetch Suggested Property Id from Property Master
                    $property = PropertyMaster::where('old_propert_id', $regUserDetails->correct_property_id)->latest('created_at')->first();
                    if (empty($property)) {
                        $splitedProperty = SplitedPropertyDetail::where('old_property_id', $regUserDetails->correct_property_id)->first();
                        if (!empty($splitedProperty)) {
                            ...
                            $data['isSplited'] = 1;
                            $property = PropertyMaster::find($splitedProperty->property_master_id);
                            $oldPropertyId = $splitedProperty?->old_property_id ? $splitedProperty->old_property_id : $property->old_property_id;
                            $suggestedPropertyId = $splitedProperty?->id ? $splitedProperty->id : $property->id;
                        }
                    }
                } else {
                    $property = PropertyMaster::where('new_colony_name', $regUserDetails->locality)->where('block_no', $regUserDetails->block)->where('plot_or_property_no', $regUserDetails->plot)->first();
                ...
                if (!empty($property['id'])) {
                    ...
                    if (isset($response) && $response->successful()) {
                        ...
                        if (!empty($jsonData)) {
                            $scannedFiles['baseUrl'] = $jsonData[0]['Path'];
                            foreach ($jsonData[0]['ListFileName'] as $files) {
                                $scannedFiles['files'][] = $files['PropertyFileNameDcrpt'];
                            }
                        } else {
                            // Handle case where the response is empty or not as expected
                            Log::warning('API response returned empty or invalid data.');
                    ...
                    // $data['oldPropertyId'] = $property['old_propert_id'];
                    $data['propertyMasterId'] = $property['id'];
                    $data['suggestedPropertyId'] = $oldPropertyId ?? $property['old_propert_id'];
                    $data['oldPropertyId'] = $oldPropertyId ?? $property['old_propert_id'];
    ...
    public function rejectUserRegisteredProperty(Request $request)
    {
        try {
            if (!empty($request->rejectUserId) && !empty($request->remarks)) {
                ...
                if (!$getDetail) {
                    return response()->json(['status' => 'failed', 'message' => 'Registration not found.']);
                }
                
                // Get the status ID for 'RS_REJ' from items table
                $rejectedStatusId = getServiceType('RS_REJ');
                
                // Check if the section_id belongs to ITC section
                $isItcSection = Section::where('id', $getDetail->section_id)
                                    ->where('section_code', 'ITC')
                                    ->exists();
                ...
                try {
                    ...
                    if ($isSuccess) {
                        //Get User Details to send rejection mail, sms & whats app - Lalit (04/March/2025)
                            ...
                            // Only create history entry if section is ITC
                            if ($isItcSection) {
                                ItCellRegTransferHistory::create([
                                    'reg_app_no'            => $getDetail->applicant_number,
                                    'old_property_id'       => $getDetail->id,
                                    'manual_locality'       => $getDetail->locality_name,
                                    'manual_block'          => $getDetail->block,
                                    'manual_plot'           => $getDetail->plot,
                                    'manual_flat'           => $getDetail->flat_no,
                                    'transfer_to_locality'  => null,
                                    'transfer_to_block'     => null,
                                    'transfer_to_plot'      => null,
                                    'transfer_to_flat'      => null,
                                    'section_code'          => 'ITC',
                                    'created_by'            => Auth::id(),
                    ...
                    DB::rollBack();
                    \Log::error('Rejection transaction failed: ' . $e->getMessage());
                    return response()->json(['status' => 'failed', 'message' => 'Database error: ' . $e->getMessage()]);
                }
            } else {
    ...
    public function propertyTransferSection(Request $request)
    {
        ...
        if (empty($request->oldPropertyId) || empty($request->sectionId) || empty($request->remarks)) { //added by Swati on 07/01/2026
            return response()->json(['status' => 'failed', 'message' => 'Property ID, Section ID & Remarks are required']);
        }

           // added by swati on 08012026 : optional document validation
        if ($request->hasFile('propertyTransferDocument')) { //added by swati on 08012026
            $request->validate([ //added by swati on 08012026
                'propertyTransferDocument' => 'file|mimes:pdf,jpg,jpeg,png|max:5120', // 5MB //added by swati on 08012026
            ]); //added by swati on 08012026
        } //added by swati on 08012026

        $searchOldId = trim((string) $request->oldPropertyId); //added by swati on 07012026

        return DB::transaction(function () use ($request, $searchOldId) { //added by Swati on 07/01/2026
            ...
            if (empty($sectionCode)) {
                return response()->json([
                    'status' => 'failed',
                    'message' => 'Section not found in master section record for selected section'
                ]);
            }

            // Resolve PropertyMaster:
            // First try split table by old_property_id  //added by Swati on 07/01/2026
            $splitRec = SplitedPropertyDetail::where('old_property_id', $searchOldId)->first();

            if (!empty($splitRec)) {
                $propertyMaster = PropertyMaster::find($splitRec->property_master_id);
                if (empty($propertyMaster)) {
                    return response()->json([
                        'status' => 'failed',
                        'message' => 'Property master not found for split property mapping'
                    ]);
                }
                $resolvedOldId = trim((string) $propertyMaster->old_propert_id);
                $locality = $propertyMaster->new_colony_name ?? '';
                $block = $propertyMaster->block_no ?? '';
                $plot = $splitRec->plot_flat_no ?? '';
            } else {
                $propertyMaster = PropertyMaster::where('old_propert_id', $searchOldId)->first();
                if (empty($propertyMaster)) {
                    return response()->json([
                        'status' => 'failed',
                        'message' => 'Property not found for given Property ID'
                    ]);
                }
                $resolvedOldId = trim((string) $propertyMaster->old_propert_id);
                $locality = $propertyMaster->new_colony_name ?? '';
            ...
            // Check if property already belongs to the selected section (null-safe)
            if (trim((string)$propertyMaster->section_code) === trim((string)$sectionCode)) {
                return response()->json([
                    'status' => 'failed',
                    'message' => 'Property already belongs to the selected section.'
                ]);
            }

            // -------------------------------
            // added by swati on 07012026 : store old section code before update
            $oldSectionCode = trim((string) $propertyMaster->section_code);
            $newSectionCode = trim((string) $sectionCode);
            // -------------------------------

            // added by swati on 08012026 : store optional transfer document
            $documentPath = null; //added by swati on 08012026
            if ($request->hasFile('propertyTransferDocument')) { //added by swati on 08012026
                $documentPath = $request->file('propertyTransferDocument') //added by swati on 08012026
                    ->store('property-transfer-docs', 'public'); //added by swati on 08012026
            } //added by swati on 08012026

            // Store original property details before updating
            $oldPropertyMaster = $propertyMaster->getOriginal();

            $propertyMaster->section_code = $newSectionCode;
            ...
            if ($propertyMaster->isDirty()) {
                ...
                // -------------------------------
                // added by swati on 07012026 : insert transfer log in property_section_transfer table
                DB::table('property_section_transfer')->insert([
                    'old_property_id'    => $resolvedOldId,
                    'old_section_code'   => $oldSectionCode,
                    'new_section_code'   => $newSectionCode,
                    'remarks'            => $request->remarks,
                    'property_transfer_document' => $documentPath, //added by swati on 08012026
                    'updated_by'         => Auth::id(),
                    'created_at'         => now(),
                    'updated_at'         => now(),
                ]);
                ...
                $propertyMasterHistory->property_master_id = $propertyMaster->id;
                ...
                //Update section code in User Property, User Registration, Application & Individual Applications.
                $sectionId = $request->sectionId;
                if (!empty($propertyMaster)) {
                    if ($propertyMaster->is_joint_property == 1) {
                        $splitRecord = SplitedPropertyDetail::where('property_master_id', $propertyMaster->id)->first();
                        if (!empty($splitRecord)) {
                            $newSearchOldPropertyId = trim((string) $splitRecord->old_property_id);
                        }
                    } else {
                        $newSearchOldPropertyId = trim((string) $propertyMaster->old_propert_id);
                    }
                }
                if (!empty($newSearchOldPropertyId)) {
                    UserProperty::where('old_property_id', $newSearchOldPropertyId)
                        ->update(['section_code' => $newSectionCode]);

                    // Update section code in user registration through correct property id
                    $updateURTCPID = UserRegistration::where('correct_property_id', $newSearchOldPropertyId);
                    if ($updateURTCPID->exists()) {
                        $updateURTCPID->update(['section_id' => $sectionId]);
                    } else {
                        // Update section code in user registration through locality, block & plot
                        $updateURTLBP = UserRegistration::where([['locality', $locality], ['block', $block], ['plot', $plot]]);
                        if ($updateURTLBP->exists()) {
                            $updateURTLBP->update(['section_id' => $sectionId]);
                        }
                    }

                    /* UserRegistration::where('correct_property_id', $newSearchOldPropertyId)
                        ->update(['section_id' => $sectionId]); */

                    // Collect application numbers and update individual records if needed
                    $applicationNumbers = collect([
                        ...
                        // GiftPermissionApplication::class,
                        // SalePermissionApplication::class,
                    ])->flatMap(function ($modelClass) use ($newSearchOldPropertyId, $sectionId) {
                        $application = $modelClass::where('old_property_id', $newSearchOldPropertyId)->first();

                        if ($application) {
                            // Only update if the application table doesn't have section_id
                            // Or remove this line if Application table update is sufficient
                            $application->update(['section_id' => $sectionId]);
                    ...
                    // Bulk update applications table
                    if (!empty($applicationNumbers)) {
                        Application::whereIn('application_no', $applicationNumbers)
                            ->update(['section_id' => $sectionId]);
                    }
                }
                ...
                return response()->json([
                    'status' => 'success',
                    'message' => 'Property successfully transferred to section',
                    'resolved_old_property_id' => $resolvedOldId
                ]);
            }

            return response()->json([
                'status' => 'failed',
                'message' => 'Transaction failed, noo changes has been made in property section transfer'
            ]);
        });
    }
    ...
    public function updateCorrectPropertyIdUserRegistrations(Request $request)
    {
        if (empty($request->registrationId) || empty($request->correctPropertyId)) {
            return response()->json([
                'status' => 'failed',
                'message' => 'Registration ID and Correct Property ID are required.'
            ]);
        }

        // Check in splited_property_details first
        $propertyExists = SplitedPropertyDetail::where('old_property_id', $request->correctPropertyId)
            ->exists();

        // If not found in splited, check in property_masters
        if (!$propertyExists) {
            $propertyExists = PropertyMaster::where('old_propert_id', $request->correctPropertyId)
                ->exists();
        }
        ...
        // Update the record
        $isUpdate = UserRegistration::where('id', $request->registrationId)
            ->update(['correct_property_id' => $request->correctPropertyId, 'correct_property_updated_by' => Auth::id()]);

        if ($isUpdate) {
            return response()->json([
                'status' => 'success',
                'message' => 'Property id successfully updated for this registrations'
            ]);
        }
    ...
    public function updateCorrectPropertyIdNewApplicantProperty(Request $request)
    {
        if (empty($request->newlyAddedPropertyId) || empty($request->correctPropertyId)) {
            return response()->json([
                'status' => 'failed',
                'message' => 'Registration ID and Correct Property ID are required.'
            ]);
        ...
        // Update the record
        $isUpdate = NewlyAddedProperty::where('id', $request->newlyAddedPropertyId)
            ->update(['correct_property_id' => $request->correctPropertyId, 'correct_property_updated_by' => Auth::id()]);

        if ($isUpdate) {
            return response()->json([
                'status' => 'success',
                'message' => 'Property id successfully updated for this new applicant property'
            ]);
        }
...
/**
 * Display IT Cell Transfer History page
 */
public function transferHistoryIndex(Request $request)
{
    ...
    return view('officials.register-users.transfer-history', compact('items', 'getStatusId', 'user'));
}

/**
 * Get transfer history data for DataTable
 */
public function getTransferHistoryData(Request $request)
{
    $user = Auth::user();
    
    // Build the query - joining with old_colonies table for transfer_to_locality and items table for status
    $query = ItCellRegTransferHistory::query()
        ->leftJoin('users as created_by_user', 'it_cell_reg_transfer_histories.created_by', '=', 'created_by_user.id')
        ->leftJoin('old_colonies as transfer_locality_table', 'it_cell_reg_transfer_histories.transfer_to_locality', '=', 'transfer_locality_table.id')
        ->leftJoin('items', 'it_cell_reg_transfer_histories.status', '=', 'items.id') // Join with items table for status
        ->select(
            'it_cell_reg_transfer_histories.*',
            'created_by_user.name as created_by_name',
            'transfer_locality_table.name as transfer_to_locality_name',
            'items.item_code as status_code' // Get the item_code for status
        )
        ->orderBy('it_cell_reg_transfer_histories.created_at', 'DESC');

    // Apply date range filter if provided
    if ($request->from_date) {
        $query->whereDate('it_cell_reg_transfer_histories.created_at', '>=', $request->from_date);
    }
    
    if ($request->to_date) {
        $query->whereDate('it_cell_reg_transfer_histories.created_at', '<=', $request->to_date);
    }
    ...
    if ($request->input('order.0.column')) {
        ...
        $order = $columns[7]; // Default sort by created_at
        $dir = 'desc';
    }
    ...
    if (!empty($request->input('search.value'))) {
        $search = $request->input('search.value');
        $query->where(function ($q) use ($search) {
            $q->where('it_cell_reg_transfer_histories.reg_app_no', 'LIKE', "%{$search}%")
                ->orWhere('it_cell_reg_transfer_histories.old_property_id', 'LIKE', "%{$search}%")
                ->orWhere(DB::raw("CONCAT_WS('/', it_cell_reg_transfer_histories.manual_locality, it_cell_reg_transfer_histories.manual_block, it_cell_reg_transfer_histories.manual_plot, it_cell_reg_transfer_histories.manual_flat)"), 'LIKE', "%{$search}%")
                ->orWhere(DB::raw("CONCAT_WS('/', transfer_locality_table.name, it_cell_reg_transfer_histories.transfer_to_block, it_cell_reg_transfer_histories.transfer_to_plot, it_cell_reg_transfer_histories.transfer_to_flat)"), 'LIKE', "%{$search}%")
                ->orWhere('it_cell_reg_transfer_histories.section_code', 'LIKE', "%{$search}%")
                ->orWhere('created_by_user.name', 'LIKE', "%{$search}%")
                ->orWhere('items.item_name', 'LIKE', "%{$search}%"); // Search by status code
        });
    ...
    $transferHistories = $query->offset($start)
        ->limit($limit)
        ->orderBy($order, $dir)
        ->get();
    ...
    $data = [];
    foreach ($transferHistories as $key => $history) {
        // Build manual entry address (manual_locality already has name, not ID)
        $manualAddressParts = [];
        if ($history->manual_locality) $manualAddressParts[] = $history->manual_locality;
        if ($history->manual_block) $manualAddressParts[] = $history->manual_block;
        if ($history->manual_plot) $manualAddressParts[] = $history->manual_plot;
        if ($history->manual_flat) $manualAddressParts[] = $history->manual_flat;
        $manualEntryAddress = !empty($manualAddressParts) ? implode('/', $manualAddressParts) : 'N/A';
        
        // Build transferred to address using locality name from old_colonies table
        $transferredAddressParts = [];
        if ($history->transfer_to_locality_name) $transferredAddressParts[] = $history->transfer_to_locality_name;
        if ($history->transfer_to_block) $transferredAddressParts[] = $history->transfer_to_block;
        if ($history->transfer_to_plot) $transferredAddressParts[] = $history->transfer_to_plot;
        if ($history->transfer_to_flat) $transferredAddressParts[] = $history->transfer_to_flat;
        $transferredToAddress = !empty($transferredAddressParts) ? implode('/', $transferredAddressParts) : 'N/A';
        
        $nestedData = [];
        $nestedData['id'] = $key + 1;
        $nestedData['reg_app_no'] = $history->reg_app_no;
        $nestedData['status_code'] = $history->status_code ?? ''; // Status code from items table
        $nestedData['old_property_id'] = $history->old_property_id ?? 'N/A';
        $nestedData['manual_entry_address'] = $manualEntryAddress;
        $nestedData['transferred_to_address'] = $transferredToAddress;
        $nestedData['section'] = $history->section_code ?? 'N/A';
        $nestedData['transferred_by'] = $history->created_by_name ?? 'N/A';
        $nestedData['created_at'] = Carbon::parse($history->created_at)->format('d-m-Y H:i:s');
        $nestedData['remarks'] = $history->remarks ?? '';
