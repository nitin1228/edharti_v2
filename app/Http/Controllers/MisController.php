...
class MisController extends Controller
{
    ...
    //For creating new Property ID - Sourav Chauhan (30 April 2026) START *******
        $max1 = DB::table('property_masters')
        ->whereRaw('old_propert_id REGEXP "^[0-9]+$"')
        ->max(DB::raw('CAST(old_propert_id AS UNSIGNED)'));

        $max2 = DB::table('splited_property_details')
            ->whereRaw('old_property_id REGEXP "^[0-9]+$"')
            ->max(DB::raw('CAST(old_property_id AS UNSIGNED)'));

        $max1 = $max1 ?? 0;
        $max2 = $max2 ?? 0;

        $newId = max($max1, $max2) + 1;
        //For creating new Property ID - Sourav Chauhan (30 April 2026) END *******
        ...
        return view('mis', compact(['colonyList', 'propertyStatus', 'landTypes', 'leaseTypes', 'propertyTypes', 'landTransferTypes', 'areaUnit','departments','regUserId','newPropUserId','newId']));
    }
            ...
            if ($user->can('view.all.details')) {
                ...
                $dataWithPagination = $dataWithSectionFilter->when($request->seach_term, function ($q) use ($request) {
                        ...
                        $q->where(function ($query) use ($request) {
                            $query->where('old_propert_id', 'like', '%' . $request->seach_term . '%')
                                ->orWhereHas('splitedProperties', function ($subQuery) use ($request) {
                                    $subQuery->where('old_property_id', 'like', '%' . $request->seach_term . '%');
                                });
                        });
...
//for checking property is available in the assigned section of user or not - SOURAV CHAUHAN (30/Dec/2024)
    public function isMisPropertyAvailable(Request $request){
        ...
        ->first(); */
        $propertyFromSplited = SplitedPropertyDetail::where('old_property_id', 'like', '%' . $request->search_term . '%')
            ->with('propertyMaster')
            ->first();

        // If not found, check in property_masters
        if (!$propertyFromSplited) {
            $isPropertyAvailable = PropertyMaster::where('old_propert_id', 'like', '%' . $request->search_term . '%')
                ->first();
        } else {
            $isPropertyAvailable = $propertyFromSplited->propertyMaster;
        }
    ...
    public function propertyChildDetails($id, MisService $misService, Request $request)
    {
        //   dd($request->all());
        ...
        if (isset($additionalDataJson)) {
            ...
            // $isMisChecked = ApplicationStatus::where('reg_app_no',$additionalData[2])->where('is_mis_checked',1)->first();
            $isMisChecked = ApplicationStatus::where([
                'reg_app_no' => $additionalData[2],
                'service_type' => $serviceType,
                'model_id' => $additionalData[1],
                'is_mis_checked' => 1
            ])->first();
            if(!$isMisChecked){
                $showApprove = true;
        ...
        return view('mis.child-preview', compact(['viewDetails', 'item', 'separatedData', 'childData', 'roles','additionalData' , 'showApprove']));
    ...
    public function editDetails(Request $request, $id, MisService $misService, ColonyService $colonyService)
    {
        ...
        // dd($conversion,$propertyLeaseDetail);
    ...
    public function getUserActionLogs(Request $request)
    {
        ...
        foreach ($getActionLogsData as $getActionLogDetail) {
            ...
            $nestedData['ip_address'] = $getActionLogDetail->ip_address;
