...
class RegisteredUserController extends Controller
{
    ...
    public function publicRegister(ColonyService $colonyService, MisService $misService)
    {
        ...
        return view('auth.public-register', compact(['colonyList', 'propertyTypes', 'countries','landTypes']));
    }
                ...
                if (isset($request->newUser)) {
                    ...
                    if ($request->newUser == 'propertyowner') {
                        ...
                        if (isset($request->propertyId)) {
                            ...
                            $landType    = $request->landTypeInvFill;
                        ...
                        if (isset($request->propertyIdOrg)) {
                            ...
                            $landType    = $request->landTypeOrgFill;
                    ...
                    if (empty($isRegistrationAvailable)) {
                        ...
                        //     ->orWhere('email', $email)
                        //     ->where('status','!=',getStatusName('RS_REJ'))
                        //     ->first();

                        $query = UserRegistration::where(function($query) use ($mobileNo, $email) {
                        ...
                        // Replace bindings in the SQL
                        // $fullSql = vsprintf(str_replace('?', "'%s'", $sql), $bindings);

                        // echo "SQL Query: " . $fullSql;

                        // Execute the query
                        $ismobileEmailAvailable = $query->first();


                        // dd($mobileNo,$email,getStatusName('RS_REJ'),$ismobileEmailAvailable);
                        ...
                        if (empty($ismobileEmailAvailable)) {
                            ...
                            if (is_null($locality)) {
                                ...
                                // Get section ID from simple query
                                $sectionId = Section::where('section_code', 'ITC')->pluck('id')->first();

                                // Convert to object format expected by your code
                                $section = (object) ['section_id' => $sectionId];
                            } else {
