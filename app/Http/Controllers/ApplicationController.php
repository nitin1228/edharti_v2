...
class ApplicationController extends Controller
{
    ...
    public function isPropertyFree(Request $request)
    {
        ...
        //     TempSubstitutionMutation::class, 
        //     TempDeedOfApartment::class, 
        //     DeedOfApartmentApplication::class, 
        //     LandUseChangeApplication::class, 
        //     TempLandUseChangeApplication::class,
        //     ConversionApplication::class,
        //     TempConversionApplication::class,
        ...
        //     if ($isPropertyIdAvailable && optional(Item::find($isPropertyIdAvailable->status))->item_code !== ['APP_WD','APP_APR','APP_REJ','APP_CAN']) {
        ...
        $allowedCodes = ['APP_WD', 'APP_APR', 'APP_REJ', 'APP_CAN'];

        // foreach ($modelsToCheck as $model) {

        //     $record = $model::where('old_property_id', $request->propertyId)->first();

        //     if (!$record) {
        //         continue;
        //     }

        //     $itemCode = optional(Item::find($record->status))->item_code;

        //     // If status code is NOT in allowed list => block
        //     if (!in_array($itemCode, $allowedCodes, true)) {
        //         return response()->json([
        //             'status'  => false,
        //             'message' => 'Application already available for the selected Property ID:- ' . $request->propertyId,
        //         ]);
        //     }
        // }
        ...
        return response()->json([
            'status'  => true,
            'message' => 'Property ID:- ' . $request->propertyId . ' is available',
        ]);
    ...
    public function getPropertyDetails(Request $request, PropertyMasterService $propertyMasertService)
    {
        ...
        //for checking is selected application is for flat or ploy - Sourav Chauhan - 19/May/2026
        $flatStatus = null;
        $getUserProperty = UserProperty::where('user_id', Auth::id())
            ->where(function ($query) use ($oldPropertyId) {

                $query->where('old_property_id', $oldPropertyId)
                    ->orWhere('splitted_property_id', $oldPropertyId);

            })
            ->first();

        $isItaFlat = !empty($getUserProperty?->flat_id);

        if ($isItaFlat) {
            $flatDetails = Flat::select('id', 'property_flat_status')
                ->find($getUserProperty->flat_id);
            $flatStatus = $flatDetails?->property_flat_status;
        }
        ...
        if ($draftApplicationPropertyId == 'false' && $updateId != 0) {
            ...
            $findProperty = $propertyMasertService->propertyFromSelected($oldPropertyId);
            ...
            if ($propertyDetails) {
                ...
                $data['status'] = $flatStatus ?? $childProperty['property_status'] ?? $propertyDetails['status'];


                if ($data['status'] == '952') {
                    //if free hold
                    // dd($propertyDetails);
                    $conversionDetails = PropertyTransferredLesseeDetail::where('property_master_id', $propertyDetails['id'])
                        ->when(!is_null($childId), function ($query) use ($childId) {
                            $query->where('splited_property_detail_id', $childId);
                        })
                        ->where('process_of_transfer', 'Conversion')->get();
    ...
    public function getPropertyCommonDetails($propertyId)
    {
        ...
        $data['area'] = $splitedProperty?->plot_area ?? $propertyLeaseDetail->plot_area_in_sqm;
        $data['presentlyKnownAs'] = $splitedProperty?->presently_known_as ?? $propertyLeaseDetail->presently_known_as;
        ...
        // $originalLessee = $propertyTransferDetaails->where('process_of_transfer', 'Original')->first(); //commented by swati mishra for showing all original lessee name by comma separated value
        $originalLessee = $propertyTransferDetaails
                                ->where('process_of_transfer', 'Original')
                                ->pluck('lessee_name')
                                ->filter()
                                ->implode(', ');
        $data['inFavourOf'] = $originalLessee;
        // dd($data);
        return $data;
    }
    ...
    public function uploadFile(Request $request, PropertyMasterService $propertyMasertService)
    {
        ...
        $findProperty = $propertyMasertService->propertyFromSelected($propertyId);
        if ($findProperty['status'] == 'error') {
            // return $countOnly ? 0 : collect(); //incase of error return empty collection
        } else {
            $propertyDetails = $findProperty['masterProperty'];
    ...
    public function getDraftApplication(Request $request, $id, ColonyService $colonyService, MisService $misService)
    {
        ...
        if (!empty($application)) {
            if (!empty($application->flat_id)) {
                $flatDetails = Flat::find($application->flat_id);
                $data['application']['flat_number'] = $flatDetails->flat_number;
            } else {
                $data['application']['flat_number'] = null;
            }
...
public function applicationsAllDetails()
    ...
    public function getHistoryApplications(Request $request)
    {
        ...
        $latestAppMovements = DB::table('application_movements as am1')
            ...
            ->join(DB::raw('
                (
                    ...
                    WHERE action = "APP_OBJ" AND application_no IS NOT NULL
                    GROUP BY application_no
                ) as am2'), function ($join) {
                $join->on('am1.application_no', '=', 'am2.application_no')
                    ->on('am1.created_at', '=', 'am2.latest_created_at');
            })
            ->where('am1.action', 'APP_OBJ');

        // Get the status ID for withdrawn applications
        $withdrawnStatus = getServiceType('APP_WD');

        // Define the first table query (Mutation Applications)
        $query1 = DB::table('mutation_applications as ma')
            ...
            ->leftJoin('old_colonies', 'pm.new_colony_name', '=', 'old_colonies.id')
            ->leftJoin('property_lease_details', 'pm.id', '=', 'property_lease_details.property_master_id')
            ->leftJoinSub($latestAppMovements, 'am', function ($join) {
                $join->on('ma.application_no', '=', 'am.application_no');
            })
            ->where('ma.created_by', '=', Auth::id())
            ->where('ma.status', '!=', $withdrawnStatus) // Exclude withdrawn applications
            ->select(
                'ma.id',
                'ma.created_at',
                'ma.application_no',
                ...
                DB::raw('case when pm.is_joint_property = 1 then spd.presently_known_as else property_lease_details.presently_known_as end as presently_known_as'),
                DB::raw('NULL as flat_id'), // Add NULL for flat_id on dated 07/01/25 By Lalit Tiwari
                DB::raw('NULL as flat_number'), // Add NULL for flat_number on dated 07/01/25 By Lalit Tiwari
                DB::raw("'MutationApplication' as model_name") // Add model_name for the first query
            );
        ...
        // Start query for Land Use Change Applications
        $query2 = DB::table('land_use_change_applications as lca')
            ->leftJoin('property_masters as pm', 'lca.property_master_id', '=', 'pm.id')
            ...
            ->where('lca.created_by', '=', Auth::id())
            ->where('lca.status', '!=', $withdrawnStatus) // Exclude withdrawn applications
            ->select(
                'lca.id',
                'lca.created_at',
                DB::raw('coalesce(lca.application_no,"0") as application_no'),
        ...
        // Conversion Applications (Deed of Apartment)
        $query3 = DB::table('deed_of_apartment_applications as doa')
            ->leftJoin('property_masters as pm', 'doa.property_master_id', '=', 'pm.id')
            ...
            ->where('doa.created_by', '=', Auth::id())
            ->where('doa.status', '!=', $withdrawnStatus) // Exclude withdrawn applications
            ->select(
                'doa.id',
                'doa.created_at',
                'doa.application_no',
        ...
        $query4 = DB::table('conversion_applications as ca')
            ->leftJoin('property_masters as pm', 'ca.property_master_id', '=', 'pm.id')
            ...
            ->where('ca.created_by', '=', Auth::id())
            ->where('ca.status', '!=', $withdrawnStatus) // Exclude withdrawn applications
            ->select(
                'ca.id',
                'ca.created_at',
                'ca.application_no',
        ...
        // NOC Applications - Lalit (19/March/2025)
        $query5 = DB::table('noc_applications as noc')
            ...
            ->where('noc.created_by', '=', Auth::id())
            ->where('noc.status', '!=', $withdrawnStatus) // Exclude withdrawn applications
            ->select(
                'noc.id',
                'noc.created_at',
                'noc.application_no',
    ...
    public function appGetFlatDetails(Request $request)
    {
        ...
        foreach ($modelsToCheck as $model) {
            ...
            $record = $model::where('old_property_id', $flatDetails->old_property_id)->where('flat_id',$request->flatId)->first();
            if (!$record) {
            continue;
            }
            // dd($record);
            try{
                ...
                if (!in_array($itemCode, $allowedCodes, true)) {
                    // return response()->json([
                    //     'status'  => false,
                    //     'message' => 'Application already available for the selected Flat ID:- ' . $request->flatId,
                    // ]);

                    $response = ['status' => false, 'message' => 'Application already available for the selected Flat ID:- ' . $flatDetails->flat_number, 'data' => NULL];
                    return $response;
                }
            }  catch (\Exception $e) {
            // Return custom error for exception
