...
class DashboardController extends Controller
{
    public function index(ColonyService $colonyService)
    {
        ...
        if ($user->hasAnyRole('super-admin', 'sub-admin', 'minister','Secretary')) {
            $data = self::getAdminData();
            return view('dashboard.admin', $data);
        } elseif ($user->hasAnyRole('section-officer', 'deputy-lndo','Joint Secretary')) {
    ...
    private function getAdminData() //ColonyService $colonyService
    {
        ...
        foreach ($revenueQueryResult as $row) {
            ...
            if (!in_array($service, $revenueData['services'])) {
        ...
        $data['revenueColors'] = ['#095859', '#cd5c5c', '#abcdef','#585acdff','#eb8a4aff','#ff28d0ff','#585acdff','#eb8a4aff','#ff28d0ff'];

        // NEW: Add total counts for records older than 30 days
        $oldRecordsCount = $this->getTotalOldPendingRecordsCount();
        $data['totalOldRegistrationsCount'] = $oldRecordsCount['total_old_registrations'];
        $data['totalOldApplicationsCount'] = $oldRecordsCount['total_old_applications'];
        $data['totalOldRecordsCount'] = $oldRecordsCount['total_old_records'];
    ...
    private function getSectionData()
    {
        ...
        if ($user->hasAnyRole('lndo', 'Joint Secretary')) {
            $sectionIdCommaSaperatedString .= ',43';
        }
...
//            "), 'pm.id', '=', 'pld.property_master_id');
 ...
 //           ->get();
        ...
        $mutationData['application_type'] = 'Mutation';
    ...
    {
        /*temporary for accounts*/ 
        if(Auth::user()->hasRole(['AO','engineer-officer']))
            return $data = ['nothingToShow' => true];
        $userId = Auth::id();
...
private function applicantData()
    ...
    private function getSectionPropertyCount($sectionId)
    {
        ...
        return DB::table('property_masters', 'pm')
...
//		dd($data['landValueData']);
    ...
    private function itCellUserData()
    {
     ...
     function getApplicationCountData($filter = null, $pendingOnly = false, $sectionIds = null)
    {
        $dateRanges = ['< 30', '30 - 60', '60 - 90', '> 90'];

        $start = null;
        $end = null;

        if (!is_null($filter)) {
            $range = $dateRanges[$filter];

            if (str_contains($range, '-')) {
                [$from, $to] = array_map('trim', explode('-', $range));

                $start = now()->subDays($to)->startOfDay();
                $end   = now()->subDays($from)->endOfDay();
            } else {
        ...
        $query =  Application::where('status', '<>', getServiceType("APP_WD"))
            ...
            ->when($pendingOnly, function ($query) {
                return $query->whereIn('status', [getServiceType('APP_NEW'), getServiceType('APP_PEN'),getServiceType('APP_IP')]);
            })
            ...
            ->select('service_type', 'status', DB::raw('COUNT(*) as count'))
            ->groupBy('service_type', 'status');
        //dd(sprintf(str_replace('?', '%s', $query->toSql()), ...$query->getBindings()));
        return $query->get();
    }

    public function applicationSummary(Request $request)
    {
        $filter = $request->dateFilter ?? null;
        $pending = isset($request->pending);
        $sectionIds = isset($request->sectionIds) ? $request->sectionIds :null;
        $raw = $this->getApplicationCountData($filter, $pending,$sectionIds);

        $services = getItemsByGroupId(17001)
            ->whereNotIn('item_code', ['RS_REG', 'OTP_LOGIN', 'APT_NEW'])
            ->sortBy('item_name');

        $rows = [];
        $totals = ['new' => 0, 'progress' => 0, 'disposed' => 0];

        foreach ($services as $service) {

            $new   = $raw->where('service_type', $service->id)->whereIn('status', [
                getServiceType('APP_NEW'),
                getServiceType('APP_PEN')
            ])->sum('count');

            $prog  = $raw->where('service_type', $service->id)->whereIn('status', [
                getServiceType('APP_IP'),
                getServiceType('APP_OBJ'),
                getServiceType('APP_HOLD')
            ])->sum('count');

            $disp  = $raw->where('service_type', $service->id)->whereIn('status', [
            ...
            $totals['new']      += $new;
            $totals['progress'] += $prog;
            $totals['disposed'] += $disp;

            $rows[] = [
                'service_code' => $service->item_code,
                'service_name' => $service->item_code == 'SUB_MUT' ? 'Mutation' : $service->item_name,
                'new' => $new,
                'progress' => $prog,
                'disposed' => $disp
            ];
    ...
    function getTotalApplicationCountByRange($pendingOnly = false, $sectionIds = null)
    {
        return Application::where('status', '<>', getServiceType("APP_WD"))
            ->when($pendingOnly, function ($query) {
            ...
            ->selectRaw("
                ...
                SUM(CASE WHEN DATEDIFF(CURDATE(), created_at) BETWEEN 0 AND 30 THEN 1 ELSE 0 END) AS days_0_30,
                SUM(CASE WHEN DATEDIFF(CURDATE(), created_at) BETWEEN 31 AND 60 THEN 1 ELSE 0 END) AS days_31_60,
                SUM(CASE WHEN DATEDIFF(CURDATE(), created_at) BETWEEN 61 AND 90 THEN 1 ELSE 0 END) AS days_61_90,
                SUM(CASE WHEN DATEDIFF(CURDATE(), created_at) > 90 THEN 1 ELSE 0 END) AS days_above_90
            ")
            ->first();
    }
    ...
    public function pendingApplicationNewSummary(Request $request = null)
    {
        $pending    =true;
        $sectionIds = getLoggedInUserSections() ?? null;

        $totals = $this->getTotalApplicationCountByRange($pending,$sectionIds);
        $data['totalPending'] = (int) $totals->total_pending;
        // dd($totals);
            $data['ranges'] = [
                '0_30_days'   => $totals->days_0_30 ?? 0,
                '31_60_days'  => $totals->days_31_60 ?? 0,
                '61_90_days'  => $totals->days_61_90 ?? 0,
                'above_90'    => $totals->days_above_90 ?? 0,
            ];
            return $data;
    }

    /**
     * Post remark for old records (both registrations and applications)
     */
    public function postOldRecordRemark(Request $request)
    {
        try {
            // Validate the request
            $request->validate([
                'record_id' => 'required|integer',
                'type' => 'required|in:registration,application',
            ...
            // Get user's sections for access control
            $userSections = DB::table('section_user')->where('user_id', auth()->id())->pluck('section_id');

            $record = null;
            $recordType = $request->type;
            $identifier = '';

            // Find record based on type
            if ($recordType === 'registration') {
                $record = UserRegistration::findOrFail($request->record_id);
                $identifier = $record->applicant_number;

                // Check access
                if (!$userSections->contains($record->section_id)) {
                    return response()->json([
                        'success' => false,
                ...
                $record = Application::findOrFail($request->record_id);
                $identifier = $record->application_no;
            ...
            // Update remark
            $record->pending_remark = $request->remark;
            $record->save();

            return response()->json([
                'success' => true,
                'message' => ucfirst($recordType) . ' remark posted successfully',
                'data' => [
                    'record_id' => $record->id,
                    'type' => $recordType,
                    ...
                    'posted_at' => now()->format('d-m-Y H:i:s')
                ]
            ]);
            ...
            Log::error('Error posting remark: ' . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Failed to post remark: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Display the old records listing page
     */
    public function oldPendingRecords()
    {
        return view('admin.applications.old-pending-records');
    }

    /**
     * Get old records data for DataTable via AJAX
     */
    public function getOldPendingRecordsData(Request $request)
    {
        try {
            $user = auth()->user();
            ...
            $sectionFilter = $request->get('section_filter', 'all'); // New section filter
            $daysThreshold = $request->get('days_threshold', 90); // Configurable days threshold

            // Process Registrations Query
            $statusIds = Item::whereIn('item_code', ['RS_PEN', 'RS_REW', 'RS_UREW', 'RS_NEW'])->pluck('id');

            // Process Applications Query
            $appStatusIds = Item::whereIn('item_code', ['APP_NEW', 'APP_PEN', 'APP_IP', 'APP_HOLD'])->pluck('id');

            // Build select queries based on type filter
            if ($typeFilter == 'registration') {
                // Only registrations
                $allRecords = $this->getRegistrations($statusIds, $dateFrom, $dateTo, $sectionFilter, $daysThreshold);
            } elseif ($typeFilter == 'application') {
                // Only applications
                $allRecords = $this->getApplications($appStatusIds, $dateFrom, $dateTo, $sectionFilter, $daysThreshold);
            } else {
                // Both registrations and applications
                $registrations = $this->getRegistrations($statusIds, $dateFrom, $dateTo, $sectionFilter, $daysThreshold);
                $applications = $this->getApplications($appStatusIds, $dateFrom, $dateTo, $sectionFilter, $daysThreshold);
                $allRecords = $registrations->concat($applications)->sortByDesc(function ($record) {
                    // Sort by the appropriate date field (last action date)
            ...
            return DataTables::of($allRecords)
                ->addIndexColumn()
                ->addColumn('days_pending', function ($row) {
                    $days = Carbon::parse($row->last_action_date)->diffInDays(now());
                    $badgeClass = $days >= 60 ? 'danger' : ($days >= 30 ? 'warning' : 'info');
                    return '<span class="badge badge-' . $badgeClass . '">' . $days . ' days</span>';
                })
                ->addColumn('last_action_date', function ($row) {
                    return $row->last_action_date ? Carbon::parse($row->last_action_date)->format('d-m-Y H:i:s') : 'N/A';
                })
                ->addColumn('pending_remark', function ($row) {
                    $remark = $row->pending_remark ?? '';
                    if (empty($remark)) {
                        return '<span class="text-muted">No remark</span>';
                    }

                    // Truncate to 20 characters
                    $shortRemark = strlen($remark) > 20 ? substr($remark, 0, 20) . '...' : $remark;

                    // Return HTML with data attributes for modal
                    return '<span class="remark-tooltip" 
                ...
                ->addColumn('action', function ($row) {
                    if ($row->record_type == 'registration') {
                        $viewUrl = route('register.user.details', $row->id);
                    } else {
                        $viewUrl = route('applications.view', $row->id);
                    }

                    return '<a href="' . $viewUrl . '" class="btn btn-sm btn-info" target="_blank">
                ...
                ->rawColumns(['days_pending', 'pending_remark', 'action'])
                ->make(true);
        } catch (\Exception $e) {
            Log::error('Error in getOldPendingRecordsData: ' . $e->getMessage());
            return response()->json(['error' => $e->getMessage()], 500);
        }
    }

    /**
     * Get registrations data with section information
     * Now uses updated_at as the last action date for registrations
     */
    private function getRegistrations($statusIds, $dateFrom = null, $dateTo = null, $sectionFilter = 'all', $daysThreshold = 30)
    {
        $query = UserRegistration::whereIn('status', $statusIds)
        ...
        $sectionIds = $registrations->pluck('section_id')->unique()->filter()->values()->toArray();

        // Get sections using DB facade
        $sections = DB::table('sections')
            ->whereIn('id', $sectionIds)
            ->get()
            ->keyBy('id');

        // Get section users with their assigned user names using DB facade
        $sectionUsers = DB::table('section_user')
            ->whereIn('section_id', $sectionIds)
            ->where('section_user.designation_id', 4)
            ->join('users', 'section_user.user_id', '=', 'users.id')
            ->select(
        ...
        return $registrations->map(function ($registration) use ($sections, $sectionUsers) {
            $sectionName = 'N/A';
            $assignedUserName = 'N/A';

            if ($registration->section_id && isset($sections[$registration->section_id])) {
                $sectionName = $sections[$registration->section_id]->name;

                // Check if section has assigned users
                if (isset($sectionUsers[$registration->section_id]) && $sectionUsers[$registration->section_id]->count() > 0) {
                    // Access user_name directly from the stdClass object
                    $assignedUserName = $sectionUsers[$registration->section_id]->first()->user_name;
                }
            }
            ...
            return (object)[
                ...
                'last_action_date' => $registration->updated_at, // Use updated_at as last action date for registrations
            ];
        });
    }

    /**
     * Get applications data with section information
     * Now uses application_movements.updated_at as the last action date for applications
     * Similar to the middleware logic
     */
    private function getApplications($appStatusIds, $dateFrom = null, $dateTo = null, $sectionFilter = 'all', $daysThreshold = 30)
    {
        $query = Application::whereIn('applications.status', $appStatusIds)
            ->whereNull('applications.disposed_at')
            ->join('application_movements', 'applications.application_no', '=', 'application_movements.application_no')
            ->where('application_movements.updated_at', '<=', now()->subDays($daysThreshold))
            ->select('applications.*', 'application_movements.updated_at as movement_updated_at')
            ->distinct();

        // Apply date filters on created_at if provided
        if ($dateFrom) {
            $query->whereDate('applications.created_at', '>=', Carbon::parse($dateFrom));
        }
        if ($dateTo) {
            $query->whereDate('applications.created_at', '<=', Carbon::parse($dateTo));
        }

        // Apply section filter
        if ($sectionFilter !== 'all') {
            $query->where('applications.section_id', $sectionFilter);
        }
        ...
        // Get unique section IDs
        $sectionIds = $applications->pluck('section_id')->unique()->filter()->values()->toArray();
        ...
        return $applications->map(function ($application) use ($users, $sections, $sectionUsers) {
            $userName = isset($users[$application->created_by]) ? $users[$application->created_by]->name : 'N/A';

            $sectionName = 'N/A';
            $assignedUserName = 'N/A';

            if ($application->section_id && isset($sections[$application->section_id])) {
                $sectionName = $sections[$application->section_id]->name;

                // Check if section has assigned users
                if (isset($sectionUsers[$application->section_id]) && $sectionUsers[$application->section_id]->count() > 0) {
                    // Access user_name directly from the stdClass object
                    $assignedUserName = $sectionUsers[$application->section_id]->first()->user_name;
                }
            }
            ...
            return (object)[
                ...
                'last_action_date' => $application->movement_updated_at, // Use movement_updated_at as last action date
            ];
        });
    }
     ...
     */
    public function getSectionsForFilter()
    {
        try {
            $specificSections = ['LS1', 'LS2A', 'LS2B', 'LS3', 'LS4', 'LS5', 'PS1', 'PS2', 'PS3', 'ITC', 'RPC']; // Define your specific section codes
            $sections = Section::select('id', 'name')
                ->whereIn('section_code', $specificSections)
                ->orderBy('id')
                ->get();
            return response()->json(['sections' => $sections]);
        } catch (\Exception $e) {
            Log::error('Error in getSectionsForFilter: ' . $e->getMessage());
            return response()->json(['error' => $e->getMessage()], 500);
        }
    }

    /**
     * Get total count of old pending records (30+ days) without section filter
     * 
     * @param int $daysThreshold (default: 30)
     * @return array
     */
    private function getTotalOldPendingRecordsCount($daysThreshold = 90)
    {
        try {
            // Process Registrations Query
            $statusIds = Item::whereIn('item_code', ['RS_PEN', 'RS_REW', 'RS_UREW', 'RS_NEW'])->pluck('id');
            ...
            // Get old registrations count (without section filter)
            $oldRegistrationsCount = UserRegistration::whereIn('status', $statusIds)
                ->where('updated_at', '<=', now()->subDays($daysThreshold))
                ->whereNull('deleted_at')
                ->count();

            // Get old applications count (without section filter)
            $oldApplicationsCount = Application::whereIn('applications.status', $appStatusIds)
                ->whereNull('applications.disposed_at')
                ->join('application_movements', 'applications.application_no', '=', 'application_movements.application_no')
                ->where('application_movements.updated_at', '<=', now()->subDays($daysThreshold))
                ->distinct()
            ...
            return [
                'total_old_registrations' => $oldRegistrationsCount,
                'total_old_applications' => $oldApplicationsCount,
                'total_old_records' => $oldRegistrationsCount + $oldApplicationsCount,
                'days_threshold' => $daysThreshold
            ];
        } catch (\Exception $e) {
            Log::error('Error in getTotalOldPendingRecordsCount: ' . $e->getMessage());
            return [
                'total_old_registrations' => 0,
                'total_old_applications' => 0,
                'total_old_records' => 0,
                'days_threshold' => $daysThreshold,
                'error' => $e->getMessage()
            ];
