...
// End
//added by swati on 10042026
$("#selpermRepeater").createRepeater({
  showFirstItemToDefault: true,
});
function getBaseURL() {
...
$(document).ready(function () {
  ...
  $("#applicationType").change(function () {
    ...
    if (selectedValue === "NOC") {
      ...
      $(".deedofappartmentDiv").hide();
      $(".landusechangeDiv").hide();
      alert("Correct SEL_PERM handler reached");
      getUserDetails()
        .then(function (response) {
          if (response.status) {
      ...
      getUserDetails()
        .then(function (response) {
          if (response.status) {
            $("#selpermNameApp").val(response.data.user.name);
            if (response.data.details.gender) {
              $("#selpermGenderApp").val(response.data.details.gender);
            } else {
              $("#selpermGenderApp").closest('.col-lg-4').hide();
            }
            if (response.data.details.dob) {
              $("#selpermDateOfBirthApp").val(response.data.details.dob);
            } else {
              $("#selpermDateOfBirthApp").closest('.col-lg-4').hide();
            }
            if (response.data.details.so_do_spouse) {
              $("#selpermPrefixApp").html(response.data.details.so_do_spouse);
            } else {
              $("#selpermPrefixApp").closest('.col-lg-4').hide();
            }
            $("#selpermAgeApp").val(response.data.details.age);
            $("#selpermFathernameApp").val(response.data.details.second_name);
            $("#isApplicantIndian").val(response.data.details.isIndian);
            if (response.data.details.isIndian == 1) {
              $("#selpermAadharApp").val(decryptString(response.data.details.aadhar_card));
              const pancard = response.data.details.pan_card
                ? response.data.details.pan_card
                : response.data.details.organization_pan_card;
              $("#selpermPanApp").val(decryptString(pancard));
            } else {
              let varDocType;
              if (response.data.details.documentType === "pion") {
                varDocType = "Person of Indian Origin Number";
              ...
              $("#selpermDocumentType").val(varDocType);
              $("#selpermDocumentTypeNumber").val(
                decryptString(response.data.details.documentTypeNumber)
              );
            }
            $("#selpermMobileApp").val(response.data.user.mobile_no);
          }
        })
        .catch(function (error) {
          console.error("Error during AJAX call:", error);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  submitButton1.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      if (applicationType === "SUB_MUT" && validateForm1MUT()) {
        ...
        //added by Swati Mishra on 10042026
        else if (applicationType === "SEL_PERM" && validateForm1SelPerm()) {
          spinner.style.display = "flex";
          btn.textContent = "Submitting.";
          btn.disabled = true;
          ...
          selpermStepFirst(propertyid, propertyStatus, function (success, result) {
            spinner.style.display = "none";
            btn.innerHTML = 'Next <i class="bx bx-right-arrow-alt ms-2"></i>';
            btn.disabled = false;
            ...
            if (success) {
              showSuccess(result.message);
              steppers["stepper8"].next();
            } else {
              showError(result.message ? result.message : result);
            }
          });
        }
      }
  ...
  submitButton2.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      //added by swati on 10042026
      else if (applicationType === "SEL_PERM" && validateForm2SelPerm()) {
        spinner.style.display = "flex";
        btn.textContent = "Submitting.";
        btn.disabled = true;

        selpermFinalStep(function (success, result) {
          spinner.style.display = "none";
  ...
  Array.from(editFinalSubmit).forEach((btn) => {
    btn.addEventListener("click", function (event) {
      ...
      var applicationType = $("select[name='applicationType']").val();
      const validTypesName = ["CONVERSION", "SUB_MUT", "NOC", "DOA", "LUC", "SEL_PERM"];
...
function noc(propertyid, propertyStatus, callback) {
  ...
  var statusofapplicant = $("#statusofapplicant").val();
  var flatid = "";
  if ($("#flatid").val() != undefined && $("#flatid").val() != "") {
    flatid = $("#flatid").val();
  }
...
// SEL_PERM Co-applicant Repeater Listeners
//added by Swati on 10042026

$(document).on(
  "blur change",
  "#selpermRepeater .coapplicant-block input:not([type='date']), #selpermRepeater .coapplicant-block select, #selpermRepeater .repeater-add-btn",
  function () {
    coapplicantSelpermRepeaterForm();
  }
);

$(document).on(
  "blur",
  "#selpermRepeater .coapplicant-block input[type='date']",
  function () {
    coapplicantSelpermRepeaterForm();
  }
);
...
$(document).on(
  ...
  "#selpermRepeater .coapplicant-block input[type='date']",
  function () {
    var selpermCoDobInput = $(this);
    var ageSelpermField = selpermCoDobInput
      .closest(".coapplicant-block")
      .find("input[id$='_age']");
    if (!selpermCoDobInput.val()) {
      ageSelpermField.val("");
    }
  }
);

// Calculate age on page load for draft/edit
$("#selpermRepeater .coapplicant-block").each(function () {
  var selpermCoDobInput = $(this).find("input[id$='_dateofbirth']");
  var ageSelpermField = $(this).find("input[id$='_age']");
  var selpermDobValue = selpermCoDobInput.val();
  if (selpermDobValue) {
    var dob = new Date(selpermDobValue);
    var age = selpermCalculateAge(dob);
    ageSelpermField.val(age);
  }
});

function coapplicantSelpermRepeaterForm() {
  var isCoapplicantSelpermValid = true;

  $("#selpermRepeater .coapplicant-block").each(function (index, element) {
    let currentIndex = $(this).data("index");

    var selpermCoName = $(element).find("#selpermcoapplicant_" + currentIndex + "_name").val();
    var selpermCoGender = $(element).find("#selpermcoapplicant_" + currentIndex + "_gender").val();
    var selpermCoDobInput = $(element).find("#selpermcoapplicant_" + currentIndex + "_dateofbirth");
    var selpermCoRelation = $(element).find("#selpermcoapplicant_" + currentIndex + "_fathername").val();
    var selpermCoAadharNumber = $(element).find("#selpermcoapplicant_" + currentIndex + "_aadharnumber").val();
    var selpermCoPanNumber = $(element).find("#selpermcoapplicant_" + currentIndex + "_pannumber").val();
    var selpermCoMobileNumber = $(element).find("#selpermcoapplicant_" + currentIndex + "_mobilenumber").val();

    var selpermCoAadharFileInput = $(element).find("#selpermcoapplicant_" + currentIndex + "_aadhaarfile");
    var selpermCoAadharFile = selpermCoAadharFileInput.length > 0 && selpermCoAadharFileInput[0].files.length > 0
      ? selpermCoAadharFileInput[0].files[0] : null;

    var selpermCoPanFileInput = $(element).find("#selpermcoapplicant_" + currentIndex + "_panfile");
    var selpermCoPanFile = selpermCoPanFileInput.length > 0 && selpermCoPanFileInput[0].files.length > 0
      ? selpermCoPanFileInput[0].files[0] : null;

    var selpermCoPhotoFileInput = $(element).find("#selpermcoapplicant_" + currentIndex + "_photo");
    var selpermCoPhotoFile = selpermCoPhotoFileInput.length > 0 && selpermCoPhotoFileInput[0].files.length > 0
      ? selpermCoPhotoFileInput[0].files[0] : null;

    const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;

    var aadhaarShouldValidate = selpermCoAadharFileInput.attr("data-should-validate");
    var panShouldValidate = selpermCoPanFileInput.attr("data-should-validate");
    var photoShouldValidate = selpermCoPhotoFileInput.attr("data-should-validate");

    var today = new Date().toISOString().split("T")[0];
    selpermCoDobInput.attr("max", today);
    var selpermCoDob = selpermCoDobInput.val();
    var ageSelpermField = $(element).find("#selpermcoapplicant_" + currentIndex + "_age");
    ...
    if (selpermCoDob) {
      var dob = new Date(selpermCoDob);
      var todayDate = new Date();
      todayDate.setHours(0, 0, 0, 0);
      if (dob > todayDate) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Date of birth cannot be in the future.", "#selpermcoapplicant_" + currentIndex + "_dateofbirth");
        selpermCoDobInput.val("");
        ageSelpermField.val("");
      } else {
        var age = selpermCalculateAge(dob);
        if (age > 100) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Age cannot be more than 100 years.", "#selpermcoapplicant_" + currentIndex + "_dateofbirth");
          selpermCoDobInput.val("");
          ageSelpermField.val("");
        } else {
          ageSelpermField.val(age);
    ...
    // If any field is filled, all become mandatory
    var isAnyFieldFilled = selpermCoName || selpermCoGender || selpermCoDob ||
      selpermCoRelation || selpermCoAadharNumber || selpermCoAadharFile ||
      selpermCoPanNumber || selpermCoPanFile || selpermCoMobileNumber || selpermCoPhotoFile;

    if (isAnyFieldFilled) {

      if (!selpermCoName) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Name is required.", "#selpermcoapplicant_" + currentIndex + "_name");
      } else if (!/^[A-Za-z\s]+$/.test(selpermCoName)) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Name must contain letters only.", "#selpermcoapplicant_" + currentIndex + "_name");
      }

      if (!selpermCoGender) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Gender is required.", "#selpermcoapplicant_" + currentIndex + "_gender");
      }

      if (!selpermCoRelation) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Relation is required.", "#selpermcoapplicant_" + currentIndex + "_fathername");
      }

      if (!selpermCoAadharNumber) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Aadhaar number is required.", "#selpermcoapplicant_" + currentIndex + "_aadharnumber");
      } else if (!/^\d{12}$/.test(selpermCoAadharNumber)) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Aadhaar number must be 12 digits.", "#selpermcoapplicant_" + currentIndex + "_aadharnumber");
      }

      // Aadhaar file
      if (aadhaarShouldValidate !== "1") {
        if (!selpermCoAadharFile) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Aadhaar PDF is required.", "#selpermcoapplicant_" + currentIndex + "_aadhaarfile");
        } else if (selpermCoAadharFile.size > 5 * 1024 * 1024) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Aadhaar file size must be less than 5MB.", "#selpermcoapplicant_" + currentIndex + "_aadhaarfile");
        } else if (!selpermCoAadharFile.name.toLowerCase().endsWith(".pdf")) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Only PDF files are allowed.", "#selpermcoapplicant_" + currentIndex + "_aadhaarfile");
        }
      }

      // PAN number (optional format check)
      selpermCoPanNumber = (selpermCoPanNumber || "").trim().toUpperCase();
      $(element)
        .find("#selpermcoapplicant_" + currentIndex + "_pannumber")
        .val(selpermCoPanNumber);

      if (selpermCoPanNumber && !panRegex.test(selpermCoPanNumber)) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(
          element,
          "PAN must be in format AAAAA9999A.",
          "#selpermcoapplicant_" + currentIndex + "_pannumber"
        );
      }

      // PAN file (optional but validate if provided)
      if (selpermCoPanFile) {
        if (selpermCoPanFile.size > 5 * 1024 * 1024) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "PAN file size must be less than 5MB.", "#selpermcoapplicant_" + currentIndex + "_panfile");
        } else if (!selpermCoPanFile.name.toLowerCase().endsWith(".pdf")) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Only PDF files are allowed.", "#selpermcoapplicant_" + currentIndex + "_panfile");
        }
      }

      if (!selpermCoMobileNumber) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Mobile number is required.", "#selpermcoapplicant_" + currentIndex + "_mobilenumber");
      } else if (!/^\d{10}$/.test(selpermCoMobileNumber)) {
        isCoapplicantSelpermValid = false;
        showErrorMessage(element, "Mobile number must be 10 digits.", "#selpermcoapplicant_" + currentIndex + "_mobilenumber");
      }

      // Passport photo
      if (photoShouldValidate !== "1") {
        if (!selpermCoPhotoFile) {
          isCoapplicantSelpermValid = false;
          showErrorMessage(element, "Co-applicant passport size photo is required.", "#selpermcoapplicant_" + currentIndex + "_photo");
        } else {
          var ext = selpermCoPhotoFile.name.split(".").pop().toLowerCase();
          if (!["jpg", "jpeg", "png"].includes(ext)) {
            isCoapplicantSelpermValid = false;
            showErrorMessage(element, "Only .jpg, .jpeg, and .png formats are allowed.", "#selpermcoapplicant_" + currentIndex + "_photo");
            selpermCoPhotoFileInput.val("");
          } else if (selpermCoPhotoFile.size > 102400) {
            isCoapplicantSelpermValid = false;
            showErrorMessage(element, "Passport photo size must be less than 100KB.", "#selpermcoapplicant_" + currentIndex + "_photo");
          }
        }
      }
    }
  ...
  return isCoapplicantSelpermValid;
}

function selpermCalculateAge(dob) {
  var today = new Date();
  var age = today.getFullYear() - dob.getFullYear();
  var m = today.getMonth() - dob.getMonth();
  if (m < 0 || (m === 0 && today.getDate() < dob.getDate())) {
...
// Start Flat Validation Code for NOC application if Flat exist By Lalit - (24/Feb/2026)
// Get flatid elements
var nocFlatid = document.getElementById("flatid");
var nocFlatidError = document.getElementById("flatidError");

// Helper function to check if element is visible
function isFlatElementVisibleForNoc(element) {
  return element && element.offsetParent !== null; // offsetParent is null if display: none
}

// Flat ID validation function
function validateNocFlatId() {
  // Skip validation if element doesn't exist or is hidden
  if (!nocFlatid || !isFlatElementVisibleForNoc(nocFlatid)) {
    // Hide error message if it exists (clean up)
    if (nocFlatidError) {
      nocFlatidError.style.display = "none";
    }
    return true; // Skip validation, consider as valid
  }

  var nocFlatidValue = nocFlatid.value.trim();
  if (nocFlatidValue === "" || nocFlatidValue === "0") {
    if (nocFlatidError) {
      nocFlatidError.textContent = "Flat ID is required.";
      nocFlatidError.style.display = "block";
    }
    return false;
  } else {
    if (nocFlatidError) {
...
// End Flat Validation Code for NOC application if Flat exist By Lalit - (24/Feb/2026)
...
function validateForm1Noc() {
  ...
  var isNocFlatId = validateNocFlatId(); // Add flatid validation By Lalit (24/Feb/2026)
  ...
  return (
    ...
    isNocRegDate &&
    isNocConvAppDate &&
    isNocApplicationType && isNocFlatId
  );
}
...
function validateConveyanceElectricityBill() {
  if (conveyanceElectricitybill.files.length > 0) {
    ...
    conveyanceElectricitybillError.textContent =
      "Latest property tax receipt PDF is required.";
    return false;
  }
}
...
// ============================================================
// SALE PERMISSION (SEL_PERM)  Step 1 & Step 2
// Swati Mishra 10042026
// ============================================================

//  Step 1 Field References 

var selpermApplicantStatus = document.getElementById("statusofapplicant"); // shared
var selpermExecutedName = document.getElementById("selpermLeaseDeedName");
var selpermDateExecutedOn = document.getElementById("selpermLeaseExecutedOn");
var selpermRegNo = document.getElementById("selpermLeaseRegnoDeed");
var selpermBookno = document.getElementById("selpermLeaseBookNoDeed");
var selpermVolumeno = document.getElementById("selpermLeaseVolumeNo");
var selpermPagenoFrom = document.getElementById("selpermLeasePagenoFrom");
var selpermPagenoTo = document.getElementById("selpermLeasePagenoTo");
var selpermRegDate = document.getElementById("selpermLeaseRegDate");

//  Step 1 Error Elements 

var selpermApplicantStatusError = document.getElementById("statusofapplicantError"); // shared
var selpermExecutedNameError = document.getElementById("selpermLeaseDeedNameError");
var selpermDateExecutedOnError = document.getElementById("selpermLeaseExecutedOnError");
var selpermRegNoError = document.getElementById("selpermLeaseRegnoDeedError");
var selpermBooknoError = document.getElementById("selpermLeaseBookNoDeedError");
var selpermVolumenoError = document.getElementById("selpermLeaseVolumeNoError");
var selpermPagenoFromError = document.getElementById("selpermLeasePagenoFromError");
var selpermPagenoToError = document.getElementById("selpermLeasePagenoToError");
var selpermRegDateError = document.getElementById("selpermLeaseRegDateError");

// Step 2 Field References

var selpermLeasedeed = document.getElementById("selpermLeasedeed");
var selpermLeasedeedError = document.getElementById("selpermLeasedeedError");
var selpermConveyancedeed = document.getElementById("selpermConveyancedeed");
var selpermConveyancedeedError = document.getElementById("selpermConveyancedeedError");
var selpermOwnershipDoc = document.getElementById("selpermConveyanceownershipdoc");
var selpermOwnershipDocError = document.getElementById("selpermConveyanceownershipdocError");
var selpermPropPhotodocFile = document.getElementById("selpermPropPhotodoc");
var selpermPropPhotodocFileError = document.getElementById("selpermPropPhotodocError");
var selpermElectricitybill = document.getElementById("selpermLatestelectricitybill");
var selpermElectricitybillError = document.getElementById("selpermLatestelectricitybillError");
var selpermAadharDoc = document.getElementById("selpermAadhardoc");
var selpermAadharDocError = document.getElementById("selpermAadhardocError");
var selpermPanDoc = document.getElementById("selpermPandoc");
var selpermPanDocError = document.getElementById("selpermPandocError");
var agreeConsentSelperm = document.getElementById("agreeConsentSelperm");
var agreeConsentSelpermError = document.getElementById("agreeConsentSelpermError");
var selpermNriUploadDocument = document.getElementById("selpermNriUploadDocument");
var selpermNriUploadDocumentError = document.getElementById("selpermNriUploadDocumentError");
var selpermDocumentType = document.getElementById("selpermDocumentType");
var selpermDocumentTypeError = document.getElementById("selpermDocumentTypeError");
var selpermDocumentTypeNumber = document.getElementById("selpermDocumentTypeNumber");
var selpermDocumentTypeNumberError = document.getElementById("selpermDocumentTypeNumberError");
var selpermAdditionalDocument = document.getElementById("selpermAdditionalDocument");
var selpermAdditionalDocumentError = document.getElementById("selpermAdditionalDocumentError");

//  Step 1 Individual Validators

function validateSelpermApplicationType() {
  var val = selpermApplicantStatus.value.trim();
  if (val === "") {
    selpermApplicantStatusError.textContent = "Applicant status is required.";
    selpermApplicantStatusError.style.display = "block";
    return false;
  } else {
    selpermApplicantStatusError.style.display = "none";
    return true;
  }
}

function validateSelpermExecutedName() {
  var val = selpermExecutedName.value.trim();
  if (val === "") {
    selpermExecutedNameError.textContent = "Executed in favour of is required.";
    selpermExecutedNameError.style.display = "block";
    return false;
  } else {
    selpermExecutedNameError.style.display = "none";
    return true;
  }
}

function validateSelpermExecutedOnDate() {
  var val = selpermDateExecutedOn.value.trim();
  var today = new Date().toISOString().split("T")[0];

  if (val === "") {
    selpermDateExecutedOnError.textContent = "Executed date is required.";
    selpermDateExecutedOnError.style.display = "block";
    return false;
  } else if (val > today) {
    selpermDateExecutedOnError.textContent = "Executed date cannot be in the future.";
    selpermDateExecutedOnError.style.display = "block";
    selpermDateExecutedOn.value = "";
    return false;
  } else {
    selpermDateExecutedOnError.style.display = "none";
    return true;
  }
}

function validateSelpermRegNo() {
  var val = selpermRegNo.value.trim();
  if (val === "") {
    selpermRegNoError.textContent = "Registration number is required.";
    selpermRegNoError.style.display = "block";
    return false;
  } else {
    selpermRegNoError.style.display = "none";
    return true;
  }
}

function validateSelpermBookno() {
  var val = selpermBookno.value.trim();

  if (val === "") {
    selpermBooknoError.textContent = "Book number is required.";
    selpermBooknoError.style.display = "block";
    return false;
  }
  if (!/^\d+$/.test(val)) {
    selpermBooknoError.textContent = "Book number must be a numeric value.";
    selpermBooknoError.style.display = "block";
    return false;
  }
  if (parseFloat(val) <= 0) {
    selpermBooknoError.textContent = "Book number must be a positive number.";
    selpermBooknoError.style.display = "block";
    return false;
  }

  selpermBooknoError.style.display = "none";
  return true;
}

function validateSelpermVolumeno() {
  var val = selpermVolumeno.value.trim();

  if (val === "") {
    selpermVolumenoError.textContent = "Volume number is required.";
    selpermVolumenoError.style.display = "block";
    return false;
  }
  if (!/^\d+$/.test(val)) {
    selpermVolumenoError.textContent = "Volume number must be a numeric value.";
    selpermVolumenoError.style.display = "block";
    return false;
  }
  if (parseFloat(val) <= 0) {
    selpermVolumenoError.textContent = "Volume must be a positive number.";
    selpermVolumenoError.style.display = "block";
    return false;
  }

  selpermVolumenoError.style.display = "none";
  return true;
}

$(document).on("input blur change", "#selpermLeasePagenoFrom", function () {
  validateSelpermPagenoFrom();
});

function validateSelpermPagenoFrom() {
  var val = selpermPagenoFrom.value.trim();
  var valTo = selpermPagenoTo.value.trim();

  if (val === "") {
    selpermPagenoFromError.textContent = "Page number from is required.";
    selpermPagenoFromError.style.display = "block";
    return false;
  }
  if (!/^\d+$/.test(val)) {
    selpermPagenoFromError.textContent = "Page number from must be a numeric value.";
    selpermPagenoFromError.style.display = "block";
    return false;
  }
  if (parseFloat(val) <= 0) {
    selpermPagenoFromError.textContent = "Page number from must be a positive number.";
    selpermPagenoFromError.style.display = "block";
    return false;
  }
  if (valTo !== "" && /^\d+$/.test(valTo) && parseInt(val) > parseInt(valTo)) {
    selpermPagenoFromError.textContent = "Page number from cannot be greater than page number to.";
    selpermPagenoFromError.style.display = "block";
    return false;
  }

  selpermPagenoFromError.style.display = "none";
  return true;
}

$(document).on("input blur change", "#selpermLeasePagenoTo", function () {
  validateSelpermPagenoTo();
});

function validateSelpermPagenoTo() {
  var val = selpermPagenoTo.value.trim();
  var valFrom = selpermPagenoFrom.value.trim();

  if (val === "") {
    selpermPagenoToError.textContent = "Page number to is required.";
    selpermPagenoToError.style.display = "block";
    return false;
  }
  if (!/^\d+$/.test(val)) {
    selpermPagenoToError.textContent = "Page number to must be a numeric value.";
    selpermPagenoToError.style.display = "block";
    return false;
  }
  if (parseFloat(val) <= 0) {
    selpermPagenoToError.textContent = "Page number to must be a positive number.";
    selpermPagenoToError.style.display = "block";
    return false;
  }
  if (valFrom !== "" && /^\d+$/.test(valFrom) && parseInt(val) < parseInt(valFrom)) {
    selpermPagenoToError.textContent = "Page number to cannot be less than page number from.";
    selpermPagenoToError.style.display = "block";
    return false;
  }

  selpermPagenoToError.style.display = "none";
  return true;
}

function validateSelpermRegDate() {
  var val = selpermRegDate.value.trim();
  var execOn = selpermDateExecutedOn.value.trim();
  var today = new Date().toISOString().split("T")[0];
  var regObj = new Date(val);
  var execObj = new Date(execOn);

  if (val === "") {
    selpermRegDateError.textContent = "Registration date is required.";
    selpermRegDateError.style.display = "block";
    return false;
  }
  if (val > today) {
    selpermRegDateError.textContent = "Registration date cannot be in the future.";
    selpermRegDateError.style.display = "block";
    selpermRegDate.value = "";
    return false;
  }
  if (execOn !== "" && regObj < execObj) {
    selpermRegDateError.textContent = "Registration cannot occur prior to the execution date.";
    selpermRegDateError.style.display = "block";
    selpermRegDate.value = "";
    return false;
  }

  selpermRegDateError.style.display = "none";
  return true;
}

// function validateSelpermConvAppDate() {
//   var val    = selpermConvAppDate.value.trim();
//   var execOn = selpermDateExecutedOn.value.trim();

//   if (execOn === "") {
//     selpermConvAppDateError.textContent = "Executed on need to be added first.";
//     selpermConvAppDateError.style.display = "block";
//     return false;
//   }
//   if (val === "") {
//     selpermConvAppDateError.style.display = "none";
//     return true; // optional field
//   }
//   var convAppDateObj  = new Date(val);
//   var executedDateObj = new Date(execOn);
//   if (convAppDateObj > executedDateObj) {
//     selpermConvAppDateError.textContent = "Registration cannot occur prior to the execution date.";
//     selpermConvAppDateError.style.display = "block";
//     selpermConvAppDate.value = "";
//     return false;
//   }
//   selpermConvAppDateError.style.display = "none";
//   return true;
...
// ConvAppDate max = ExecutedOn date (same as NOC pattern)
// document.addEventListener("DOMContentLoaded", function () {
//   var selpermExecOnInput     = document.getElementById("selpermConveyanceExecutedOn");
//   var selpermConAppDateInput = document.getElementById("selpermConveyanceConAppDate");
//   var appTypeSelect          = document.getElementById("applicationType");

//   function setSelpermMaxDate() {
//     if (selpermExecOnInput && selpermConAppDateInput) {
//       var execDate = selpermExecOnInput.value.trim();
//       if (execDate !== "") {
//         selpermConAppDateInput.setAttribute("max", execDate);
//         if (
//           selpermConAppDateInput.value &&
//           new Date(selpermConAppDateInput.value) > new Date(execDate)
//         ) {
//           selpermConAppDateInput.value = "";
//         }
//       }
...
//       if (this.value === "SEL_PERM") {
//         setSelpermMaxDate();
//       } else {
//         if (selpermConAppDateInput) {
//           selpermConAppDateInput.removeAttribute("max");
//         }
//       }
//     });
//   }
...
//   if (selpermConAppDateInput) {
//     selpermConAppDateInput.addEventListener("change", function () {
//       if (selpermConvAppDateError) {
//         selpermConvAppDateError.textContent = "";
//         selpermConvAppDateError.style.display = "none";
//       }
...
//         if (selpermConvAppDateError) {
//           selpermConvAppDateError.textContent =
//             "Selected date cannot be after the executed on date (" + formattedMaxDate + ").";
//           selpermConvAppDateError.style.display = "block";
//         }
//         this.value = "";
//       }
...
//   if (selpermExecOnInput) {
//     selpermExecOnInput.addEventListener("change", setSelpermMaxDate);
//   }
// });

// Set max = today for selpermRegDate
document.addEventListener("DOMContentLoaded", function () {
  var today = new Date().toISOString().split("T")[0];
  if (selpermRegDate) {
    selpermRegDate.setAttribute("max", today);
  }
});

// Sale Permission NRI field live validation
$(document).on("change", "#selpermDocumentType", function () {
  validateSelpermDocumentType();
});

$(document).on("input blur change", "#selpermDocumentTypeNumber", function () {
  validateSelpermDocumentTypeNumber();
});

$(document).on("change", "#selpermNriUploadDocument", function () {
  validateSelpermNriUploadDocument();
});

//  validateForm1SelPerm 

function validateForm1SelPerm() {
  var isCoapplicantValid = coapplicantSelpermRepeaterForm();
  var isExecutedNameValid = validateSelpermExecutedName();
  var isExecutedOnDateValid = validateSelpermExecutedOnDate();
  var isRegNoValid = validateSelpermRegNo();
  var isBooknoValid = validateSelpermBookno();
  var isVolumenoValid = validateSelpermVolumeno();
  var isPagenoFromValid = validateSelpermPagenoFrom();
  var isPagenoToValid = validateSelpermPagenoTo();
  var isRegDateValid = validateSelpermRegDate();
  var isApplicationTypeValid = validateSelpermApplicationType();

  return (
    isCoapplicantValid &&
    isExecutedNameValid &&
    isExecutedOnDateValid &&
    isRegNoValid &&
    isBooknoValid &&
    isVolumenoValid &&
    isPagenoFromValid &&
...
// Edit mode — skips applicant status
function validateEditSelPerm1() {
  var isCoapplicantValid = coapplicantSelpermRepeaterForm();
  var isLeaseNameValid = validateSelpermLeaseDeedName();
  var isExecutedOnValid = validateSelpermLeaseExecutedOn();
  var isRegNoValid = validateSelpermRegNo();
  var isBooknoValid = validateSelpermBookno();
  var isVolumenoValid = validateSelpermVolumeno();
  var isPagenoFromValid = validateSelpermPagenoFrom();
...
//  Step 2 Individual File Validators

function validateSelpermLeasedeed() {
  if (selpermLeasedeed && selpermLeasedeed.files.length > 0) {
    var file = selpermLeasedeed.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermLeasedeedError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermLeasedeedError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermLeasedeedError.textContent = "";
      return true;
    }
  } else {
    if (selpermLeasedeed && selpermLeasedeed.getAttribute("data-should-validate") == 1) {
      return true;
    }
    if (selpermLeasedeedError) selpermLeasedeedError.textContent = "Lease deed PDF is required.";
    return false;
  }
}

function validateSelpermPropPhotodocFile() {
  if (selpermPropPhotodocFile && selpermPropPhotodocFile.files.length > 0) {
    var file = selpermPropPhotodocFile.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermPropPhotodocFileError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermPropPhotodocFileError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermPropPhotodocFileError.textContent = "";
      return true;
    }
  } else {
    if (selpermPropPhotodocFile && selpermPropPhotodocFile.getAttribute("data-should-validate") == 1) {
      return true;
    }
    if (selpermPropPhotodocFileError) selpermPropPhotodocFileError.textContent = "Photo of the property PDF is required.";
    return false;
  }
}

function validateSelpermElectricityBill() {
  if (selpermElectricitybill && selpermElectricitybill.files.length > 0) {
    var file = selpermElectricitybill.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermElectricitybillError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermElectricitybillError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermElectricitybillError.textContent = "";
      return true;
    }
  } else {
    if (selpermElectricitybill && selpermElectricitybill.getAttribute("data-should-validate") == 1) {
      return true;
    }
    if (selpermElectricitybillError) selpermElectricitybillError.textContent = "Latest property tax receipt PDF is required.";
    return false;
  }
}

function validateSelpermAadharDoc() {
  if (selpermAadharDoc && selpermAadharDoc.files.length > 0) {
    var file = selpermAadharDoc.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermAadharDocError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermAadharDocError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermAadharDocError.textContent = "";
      return true;
    }
  } else {
    if (selpermAadharDoc && selpermAadharDoc.getAttribute("data-should-validate") == 1) {
      return true;
    }
    if (selpermAadharDocError) selpermAadharDocError.textContent = "Aadhaar PDF is required.";
    return false;
  }
}

function validateSelpermPanDoc() {
  if (selpermPanDoc && selpermPanDoc.files.length > 0) {
    var file = selpermPanDoc.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermPanDocError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermPanDocError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermPanDocError.textContent = "";
      return true;
    }
  } else {
    if (selpermPanDoc && selpermPanDoc.getAttribute("data-should-validate") == 1) {
      return true;
    }
    if (selpermPanDocError) selpermPanDocError.textContent = "PAN PDF is required.";
    return false;
  }
}

function validateAgreeConsentSelperm() {
  if (!agreeConsentSelperm.checked) {
    agreeConsentSelpermError.textContent = "Please accept terms & conditions.";
    agreeConsentSelpermError.style.display = "block";
    return false;
  } else {
    agreeConsentSelpermError.style.display = "none";
    return true;
  }
}

function validateSelpermDocumentType() {
  if (!selpermDocumentType) return true;
  var val = selpermDocumentType.value.trim();
  if (val === "") {
    selpermDocumentTypeError.textContent = "Document type is required.";
    selpermDocumentTypeError.style.display = "block";
    return false;
  } else {
    selpermDocumentTypeError.style.display = "none";
    return true;
  }
}

function validateSelpermNriUploadDocument() {
  if (!selpermNriUploadDocument) return true;
  if (selpermNriUploadDocument.files.length > 0) {
    var file = selpermNriUploadDocument.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermNriUploadDocumentError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermNriUploadDocumentError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermNriUploadDocumentError.textContent = "";
      return true;
    }
  } else {
    if (selpermNriUploadDocument.getAttribute("data-should-validate") == 1) {
      return true;
    }
    selpermNriUploadDocumentError.textContent = "Proof of NRI PDF is required.";
    return false;
  }
}

function validateSelpermDocumentTypeNumber() {
  if (!selpermDocumentTypeNumber) return true;
  var val = selpermDocumentTypeNumber.value.trim();

  if (val === "") {
    selpermDocumentTypeNumberError.textContent = "Document type number is required.";
    selpermDocumentTypeNumberError.style.display = "block";
    return false;
  }
  if (val.length < 6 || val.length > 20) {
    selpermDocumentTypeNumberError.textContent = "Document type number must be between 6 and 20 characters.";
    selpermDocumentTypeNumberError.style.display = "block";
    return false;
  }
  ...
  if (!allowedPattern.test(val)) {
    selpermDocumentTypeNumberError.textContent = "Document type number must contain only letters, numbers, spaces, dots, and hyphens.";
    selpermDocumentTypeNumberError.style.display = "block";
    return false;
  } else {
    selpermDocumentTypeNumberError.style.display = "none";
    return true;
  }
}

function validateSelpermAdditionalDocument() {
  if (!selpermAdditionalDocument) return true;

  if (selpermAdditionalDocument.files.length > 0) {
    var file = selpermAdditionalDocument.files[0];
    if (file.size > 5 * 1024 * 1024) {
      selpermAdditionalDocumentError.textContent = "File size must be less than 5 MB.";
      return false;
    } else if (!file.name.endsWith(".pdf")) {
      selpermAdditionalDocumentError.textContent = "Only PDF files are allowed.";
      return false;
    } else {
      selpermAdditionalDocumentError.textContent = "";
      return true;
    }
  } else {
    if (selpermAdditionalDocument.getAttribute("data-should-validate") == 1) {
      return true;
    }
    selpermAdditionalDocumentError.textContent = "Additional document PDF is required.";
    return false;
  }
}

//  validateForm2SelPerm 

function validateForm2SelPerm() {
  var isLeasedeed = validateSelpermLeasedeed();
  var isPropPhoto = validateSelpermPropPhotodocFile();
  var isElecBill = validateSelpermElectricityBill();
  var isAdditionalDoc = validateSelpermAdditionalDocument();
  var isConsent = validateAgreeConsentSelperm();
  var isAppOtherDoc = validateAppOtherDoc("SEL_PERM-2");
  var isPoAValid = validatePOADoc("SEL_PERM-2");
  var isApplicantIndian = document.getElementById("isApplicantIndian").value;

  if (isApplicantIndian == 1) {
    var isAadharDoc = validateSelpermAadharDoc();
    var isPanDoc = validateSelpermPanDoc();
    return (
      isLeasedeed &&
      isPropPhoto &&
      isElecBill &&
    ...
    var isDocumentType = validateSelpermDocumentType();
    var isNriUploadDocument = validateSelpermNriUploadDocument();
    var isDocumentTypeNumber = validateSelpermDocumentTypeNumber();
    return (
      isLeasedeed &&
      isPropPhoto &&
      isElecBill &&
      ...
      isDocumentType &&
      isNriUploadDocument &&
      isDocumentTypeNumber
    );
  }
}


//  Step 1 AJAX selpermStepFirst() 

function selpermStepFirst(propertyid, propertyStatus, callback) {
  var updateId = $("input[name='updateId']").val();
  var statusofapplicant = $("#statusofapplicant").val();
  var flatid = "";
  ...
  var selpermNameApp = $("input[name='selpermNameApp']").val();
  var selpermGenderApp = $("input[name='selpermGenderApp']").val();
  var selpermDateOfBirthApp = $("input[name='selpermDateOfBirthApp']").val();
  var selpermAgeApp = $("input[name='selpermAgeApp']").val();
  var selpermFathernameApp = $("input[name='selpermFathernameApp']").val();
  var selpermAadharApp = $("input[name='selpermAadharApp']").val();
  var selpermPanApp = $("input[name='selpermPanApp']").val();
  var selpermMobileApp = $("input[name='selpermMobileApp']").val();

  var selpermLeaseDeedName = $("input[name='selpermLeaseDeedName']").val();
  var selpermLeaseExecutedOn = $("input[name='selpermLeaseExecutedOn']").val();
  var selpermLeaseRegnoDeed = $("input[name='selpermLeaseRegnoDeed']").val();
  var selpermLeaseBookNoDeed = $("input[name='selpermLeaseBookNoDeed']").val();
  var selpermLeaseVolumeNo = $("input[name='selpermLeaseVolumeNo']").val();
  var selpermLeasePagenoFrom = $("input[name='selpermLeasePagenoFrom']").val();
  var selpermLeasePagenoTo = $("input[name='selpermLeasePagenoTo']").val();
  var selpermLeaseRegDate = $("input[name='selpermLeaseRegDate']").val();
  ...
  formData.append("flatid", flatid);
  formData.append("statusofapplicant", statusofapplicant);
  formData.append("selpermNameApp", selpermNameApp);
  formData.append("selpermGenderApp", selpermGenderApp);
  formData.append("selpermDateOfBirthApp", selpermDateOfBirthApp);
  formData.append("selpermAgeApp", selpermAgeApp);
  formData.append("selpermFathernameApp", selpermFathernameApp);
  formData.append("selpermAadharApp", selpermAadharApp);
  formData.append("selpermPanApp", selpermPanApp);
  formData.append("selpermMobileApp", selpermMobileApp);
  formData.append("selpermLeaseDeedName", selpermLeaseDeedName);
  formData.append("selpermLeaseExecutedOn", selpermLeaseExecutedOn);
  formData.append("selpermLeaseRegnoDeed", selpermLeaseRegnoDeed);
  formData.append("selpermLeaseBookNoDeed", selpermLeaseBookNoDeed);
  formData.append("selpermLeaseVolumeNo", selpermLeaseVolumeNo);
  formData.append("selpermLeasePagenoFrom", selpermLeasePagenoFrom);
  formData.append("selpermLeasePagenoTo", selpermLeasePagenoTo);
  formData.append("selpermLeaseRegDate", selpermLeaseRegDate);

  // Co-applicants
  $("input[name^='selpermcoapplicant'], select[name^='selpermcoapplicant']").each(function () {
    var nameAttr = $(this).attr("name");
    var value = $(this).val();
    var matches = nameAttr.match(/coapplicant\[(\d+)]\[(\w+)\]/);
    ...
    if (matches) {
      ...
      if ($(this).attr("type") == "file") {
        ...
        if (fileInput) {
          formData.append(`coapplicants[${index}][${field}]`, fileInput);
        }
      }
    }
  });
  ...
  var baseUrl = getBaseURL();
  $.ajax({
    url: baseUrl + "/selPerm-step-first",
    type: "POST",
    dataType: "JSON",
    data: formData,
    processData: false,
...
//  Final Step AJAX selpermFinalStep()

function selpermFinalStep(callback) {
  var updateId = $("input[name='updateId']").val();
  var agreeConsent = $("#agreeConsentSelperm").is(":checked") ? 1 : 0;
  var baseUrl = getBaseURL();
  var csrfToken = $('meta[name="csrf-token"]').attr("content");
  var formData = new FormData();
  ...
  formData.append("updateId", updateId);
  formData.append("agreeConsent", agreeConsent);

  // Required / optional dynamic document fields for Sale Permission step 2
  $("#SEL_PERM-2")
    .find("input[type='file'], input[type='hidden'], input[type='text'], input[type='date'], select, textarea")
    .each(function () {
      var el = $(this);
  ...
  $.ajax({
    url: baseUrl + "/selPerm-final-step",
    type: "POST",
    dataType: "JSON",
    data: formData,
    processData: false,
    ...
    error: (response) => {
      if (response.status == 422) {
        let er = response.responseJSON;
        if (callback) callback(false, er.id + "-" + er.message);
      } else {
        if (callback) callback(false, response.responseText);
      }
    },
  });
}

// End Sale Permission (SEL_PERM) 