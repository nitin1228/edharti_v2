...
@endif
						     ...
						     <b>Phone:</b> {{$data['master']['phone_no'] ?? "N/A"}}<br>
						      ...
						      <b>As on Date</b> {{ !empty($data['master']->as_on_date)  ? \Carbon\Carbon::parse($data['master']->as_on_date)->format('d-m-Y')  : 'N/A' }}
						      </td>
						      <td><b>Lease Type :</b> {{getServiceNameById($data['master']->propertyLeaseDetail->type_of_lease) ?? 'N/A'}}<br>
						      	...
						      	<b>Date of Expiration :</b> {{ !empty($data['master']->propertyLeaseDetail->date_of_expiration)  ? \Carbon\Carbon::parse($data['master']->propertyLeaseDetail->date_of_expiration)->format('d-m-Y')  : 'N/A' }}<br>
						      	<b>Date of Conveyance Deed</b> {{ !empty($data['master']->propertyLeaseDetail->date_of_conveyance_deed)  ? \Carbon\Carbon::parse($data['master']->propertyLeaseDetail->date_of_conveyance_deed)->format('d-m-Y')  : 'N/A' }}						      	
						      </td>
						    </tr>
						    <tr>						    	
                            <td><b>Date of Last Inspection Report:</b> {{ !empty($data['master']->last_inspection_ir_date)  ? \Carbon\Carbon::parse($data['master']->last_inspection_ir_date)->format('d-m-Y')  : 'N/A' }}<br>
								 <b>Date of Last Demand Letter: </b>{{ !empty($data['master']->last_demand_letter_date)  ? \Carbon\Carbon::parse($data['master']->last_demand_letter_date)->format('d-m-Y')  : 'N/A' }}<br>
								 <b>Demand ID: </b> {{$data['master']['last_demand_id'] ?? "N/A"}}<br>
								 <b>Amount of Last Demand Letter:</b>  ₹ {{customNumFormat($data['master']['last_demand_amount']) ?? 0}}<br>
                                  <b>Last Amount Received:</b> ₹ {{customNumFormat($data['master']['last_amount_received']) ?? 0}}	<br>
                                   <b>Total Dues :</b> ₹ {{customNumFormat($data['master']['total_dues']) ?? 0}}	<br>
                                  <b>Date of Last Amount Received:</b> {{ !empty($data['master']->last_amount_received_date)  ? \Carbon\Carbon::parse($data['master']->last_amount_received_date)->format('d-m-Y')  : 'N/A' }}
                            </td>
						    </tr>
							...
							<td>{{$child->scanned_files_count ?? 0}}</td>
                             ...
                             <tr>
                                <th>Property Contact Details</th>
                                  <td>
                              <b>Address:</b>aaa {{$child->propertyContactDetail->address ?? "N/A"}}<br>
						     <b>Phone:</b> {{$child->propertyContactDetail->phone_no ?? "N/A"}}<br>
						      <b>Email:</b> {{$child->propertyContactDetail->email ?? "N/A"}}<br>
						      <b>As on Date:</b> {{$child->propertyContactDetail->as_on_date ?? "N/A"}}
						      </td>
						      </tr>
                        </table>
