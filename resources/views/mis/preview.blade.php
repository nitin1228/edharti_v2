    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered property-table-info">
                                            <tbody>
                                                <tr>
                                                    <th>Flat Id :</th>
                                                    <td>{{ $flatData['flatDetails']->unique_flat_id ?? '' }}</td>
                                                    <th>File Number :</th>
                                                    <td>{{ $flatData['flatDetails']->unique_file_no ?? '' }}</td>
                                                </tr>
                                                <tr>
                                                    <th>Floor :</th>
                                                    <td>{{ $flatData['flatDetails']->floor ?? '' }}</td>
                                                    <th>Flat Number :</th>
                                                    <td>{{ $flatData['flatDetails']->flat_number ?? '' }}</td>
                                                </tr>
                                                <tr>
                                                    <th>Flat Status :</th>
                                                    <td>{{ optional($flatData['flatDetails'])->property_flat_status == 951 ? 'Lease Hold' : 'Free Hold' }}
                                                    </td>
                                                    <th>Flat Area in Sqm :</th>
                                                    <td>{{ $flatData['flatDetails']->area_in_sqm ?? '' }}</td>
                                                </tr>
                                                <tr>
                                                    <th>Purchase Date :</th>
                                                    <td>{{ !empty($flatData['flatDetails']->purchase_date) ? \Carbon\Carbon::parse($flatData['flatDetails']->purchase_date)->format('d-m-Y') : '' }}
                                                    </td>
                                                    <th>Original Buyer :</th>
                                                    <td>{{ $flatData['flatDetails']->original_buyer_name ?? '' }}</td>
                                                </tr>
                                                <tr>
                                                    <th>Present Occupant :</th>
                                                    <td>{{ $flatData['flatDetails']->present_occupant_name ?? '' }}</td>
                                                    <th>Builder / Developer Name :</th>
                                                    <td>{{ $flatData['flatDetails']->builder_developer_name ?? '' }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
            ...
            <!-- <div class="container"> -->
                ...
                {{-- Allow ASO to check Approve MIS checked on dated 06/March/2026 By Lalit --}}
                @if (($roles === 'section-officer' || $roles === 'assistant-section-officer') && auth()->user()->can('can.approve.application.mis.scanned.document'))            
                    @if ($isChecked == 1)
