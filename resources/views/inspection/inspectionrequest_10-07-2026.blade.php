      ...
      <input type="text" class="form-control"value="{{ $inspection->inspection_id ?? '' }}" readonly>
        ...
        @endif
			...
			@endphp
				...
				<div class="row mb-2 align-items-center">				   
				      <div class="col-md-2"> <b>Breach Clouse</b> <span class="text-danger">*</span> </div>
				    <div class="col-md-3">  <input type="text" class="form-control " value="{{ $address->bclouse ??''}}" id="bclouse">  </div>
				     <div class="col-md-2  text-end"> <b>Re-enter Clouse: </b> <span class="text-danger">*</span> </div>
				    <div class="col-md-3"> <input type="text" class="form-control "  value="{{ $address->bclouse ??''}}" id="breenter">  </div>
				</div>
				<div class="row mb-2 align-items-center">				   
	    	 ...
	    	 <tbody id="breachTableBody">	    	 
					@if(isset($inspectionReportBind) && $inspectionReportBind->breaches->count() > 0)
					    ...
					    <tr class="breachRow" data-changed="0">
					        <td>
					        <input type="hidden"  class="breach_id"  value="{{$breach->id ?? ''}}">
	...
	<div class="text-end">
		...
		<table class="table table-bordered" id="unauthorisedTable">
		    ...
		    <tbody id="unauthorisedTableBody">
             ...
             <td>
             <input type="hidden"    class="unauthorised_id"    value="{{$unauthorised->id ?? ''}}">
...
function(){
	...
	$(document).ready(function () {
	    ...
	    function setupDynamicTable(
	        ...
	        $(document).on("click", addButton, function () {
	           ...
	           // alert(saveUrl);
	           // return false;
	            if(saveUrl.includes("breach"))
	            {	           
				let row = $(".breachRow:last");
				$.ajax({
				    ...
				    success:function(res){
				        ...
				        row.find(".breach_id").val(res.id);
                          row.attr("data-changed","0");
				        // ab new blank row append kar do
				...
				$.ajax({
				    ...
				    success:function(res){
				        ...
				        // Iske baad blank row append kar sakte ho.
	        ...
	        $(document).on("click", removeButtonClass, function () {
			    ...
			    "#breachTableBody",
			    ".removeRow",
			    "{{ route('inspection.saveBreachRow') }}"
			);
			...
			setupDynamicTable(
			    ...
			    "{{ route('inspection.saveUnauthorisedRow') }}"
			);
 ...
 <script>
    ...
    $('select[name="objectionable[]"]').each(function () {
        ...
        if (objectionable.val() == '0' && reason.val().trim() == '') {
	...
	$("#submitButton").click(function () {
		  $(".error").html("");
			...
			{ //step -4
				...
				$(".unauthorisedRow").each(function () {
				    let wplVal  = $(this).find('[name="wpl[]"]').val().trim();
				    let bplVal  = $(this).find('[name="bpl[]"]').val().trim();
				    let areaVal = $(this).find('[name="unauthorised_area[]"]').val().trim();

				    let wpl  = parseFloat(wplVal) || 0;
				    let bpl  = parseFloat(bplVal) || 0;
				    let area = parseFloat(areaVal) || 0;

				    let areaInput = $(this).find('[name="unauthorised_area[]"]');

				    areaInput.removeClass("is-invalid");
				    areaInput.next(".error").html("");

				    // Validation tabhi chalegi jab WPL ya BPL me value ho
				    if (wplVal !== "" || bplVal !== "") {
...
//			console.log("Step3", $("#refused_by").length);
		...
		let formData = { 
		    ...
		    $("#breachTableBody tr").each(function () {
		        breachDetails.push({
		        	id : $(this).find('.breach_id').val(),
   					is_changed : $(this).attr('data-changed'),
		    ...
		    $("#unauthorisedTableBody tr").each(function () {
		        unauthorisedDetails.push({
		        	 id : $(this).find('.unauthorised_id').val(),
    				is_changed : $(this).attr('data-changed'),
