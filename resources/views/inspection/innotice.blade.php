...
<body>
    ...
    @php
		...
		<div class="content" style="font-size:14px; line-height:21px;">
		    <p> To,
		        <br>
		    ...
		    <p>Sir/Madam,<br>
		    I am directed to say that  Sh. {{ $address->inspection_officer ?? 'N/A' }}, JUNIOR ENGINEER of this office is deputed to inspect the premises of above mentioned site on {{ !empty($inspection->schedule_date) ? \Carbon\Carbon::parse($inspection->schedule_date)->format('d-M-Y') : 'N/A' }}  between {{ !empty($inspection->schedule_fromtime)  ? \Carbon\Carbon::parse($inspection->	schedule_fromtime)->format('h:i A') : 'N/A' }} and {{ !empty($inspection->schedule_totime) ? \Carbon\Carbon::parse($inspection->schedule_totime)->format('h:i A') : 'N/A' }}.
		   <br>
		    (2) You are therefore requested to be available at site on the scheduled date or inform your representative to be present at site to facilitate the inspecting officer for smooth inspection of the premises.<br>
		  The following documents should also be made available to the inspecting officer: <br>
		   (a) Certified Copy of the Sanctioned Plan.<br> (b) Completion Plan from the Local Body.<br>
		   (3) You are also requested to provide necessary access to all parts of the premises and extend full co-operation during inspection.<br>
		   (4) In case of any clarification in connection with the notice, you may contact the office on all working days. <br>
		    (5) The applicant is required to keep one certified copy of the lease readily available.<br>
		   (6) In case no lease has been executed, the applicant must produce the allotment letter.<br>
		   (7) The applicant must have the complete property chain available. </p>
		</div>
