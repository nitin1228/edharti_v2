            ...
            <div class="table-responsive mt-2">
                    ...
                    @if($query_type == "processingfee")
                          <table class="table table-bordered" id="tab-all-applications">
					        <thead>
       						...
       						<tbody>
                                ...
                                <tr>
                                   ...
                                   <td>  @if($app->status == "1546")<a href="{{route('downloadPaymentReceiptPdf',$app->unique_payment_id)}}">  
							...
							@elseif($query_type == "summary_report")
                          <table class="table table-bordered" id="tab-all-applications">
					        <thead>
					            <tr class="table-success">
					                <th>S. No.</th>
					...
					@elseif($query_type == 'summaryallreport')	
					<table class="table table-bordered" id="tab-all-applications">
						    <thead>
						        <tr class="table-success">
						            <th>S. No.</th>
    ...
    <tbody>
        ...
        @forelse($applications as $row)
            <tr>
                ...
                {{getServiceNameById($row->subhead_id) }}
                {{getServiceNameById($row->type) == 'Application' ? "Application Processing Fee" : (getServiceNameById($row->type) =='Demand' ?  getServiceNameById($row->type) : getServiceNameById($row->type)) }} 
                </td>
...
@elseif($query_type == "summaryallold")						
							<table class="table table-bordered" id="tab-all-applications">
						    <thead>
						        <tr class="table-success">
						            <th>S. No.</th>
 ...
 @elseif($query_type == "eDhartiolddemands")
                          <table class="table table-bordered" id="tab-all-applications">
					        <thead>
					            <tr class="table-success">
					                <th>S. No.</th>
       						...
       						<tbody>
                                @forelse ($applications as $app)
                                <tr class="@if($app->data_source == 'OLD') lightred @else lightgreen @endif" >
                                    <td>{{$loop->iteration}} </td>
                                    <td>{{$app->unique_propert_id ?? 'N/A'}} <br><small>({{$app->old_propert_id ?? 'N/A'}}) </small></td>
                                    <td>{{$app->DemandID ??'N/A'}}<br>
                                      <a href="javascript:void(0);" 
								       class="text-primary viewdemanddetails" 
								       data-id="{{ $app->DemandID }}" 
								       data-toggle="tooltip" 
									   data-placement="top" 
                                      ...
                                      <td>{{$app->unique_payment_id ??'N/A'}}</td>    
                                        <td>{{$app->transaction_id ??'N/A'}}</td>                                         
                                     <td>{{isset($app->updated_at)? date('d-m-Y',strtotime($app->updated_at)):"N/A" }}</td>    
                                      <td>  &#8377; {{customNumFormat($app->paid_amount)}}</td>                                   
                                     <td>{{$app->first_name}}<br>
                                     	{{$app->email}}<br>
                                     	{{$app->mobile}}
                                     </td>
                            ...
                            <tfoot>
							    <tr class="table-secondary">
							        ...
							        <th class="text-wrap" style="max-width: 150px; white-space: normal;">
							            ₹ {{ customNumFormat(round(collect($applications)->sum('paid_amount'), 2)) }}<br>           
							           {{-- {{ collect($applications)->sum('amount') > 0 
							                ? convertToIndianCurrencyWords(round(collect($applications)->sum('paid_amount'), 2)) 
							                : 'Zero Rupees Only' 
							            }} --}}
							        </th> 
							        <th colspan="4"></th>
				        ...
				        <tbody>				       
				            @forelse($applications as $demand)
				                <tr>
				                    ...
				                    @if( $demand->unique_id != "")                 
				                    <a href="{{route('ViewDemand',$demand->demaid)}}"  data-toggle="tooltip" 
									   data-placement="top" 
									   title="View Details">{{$demand->unique_id}}</a>
				                     <a href="{{ asset('storage/public' . $demand->demand_path) }}" 
									   target="_blank"
									   data-toggle="tooltip" 
									   data-placement="top" 
				                    ...
				                    <td>{{ date('d-m-Y',strtotime($demand->updated_at)) }}</td>
				                    ...
				                    <td>₹ {{ customNumFormat(round($demand->balance_amount, 2)) ?? 0 }}</td>	
                                     ...
                                     <td>
                                        @if($demand->unique_id != "")
											    {!! $demand->unique_payment_ids 
										        ? str_replace(',', '<br>', $demand->unique_payment_ids) 
										        : 'N/A' !!}
											    @else 
											     {{$demand->unique_payment_id ?? 'N/A'}}
											    @endif
											</td>
											<td> @if($demand->unique_id != "")
											{!!  $demand->transaction_ids  
										        ? str_replace(',', '<br>', $demand->transaction_ids ) 
										        : 'N/A' !!}											   
											    @else
											     {{$demand->transaction_id ?? 'N/A'}}
											     @endif
											</td>	 
				        ...
				        <tbody>				       
				            @forelse($applications as $demand)
				                <tr>
				                    ...
				                    <td>{{ $demand->balance_amount == 0 ?"Success" : getServiceNameById($demand->status) }}</td>     
                                    <td>  @if($demand->status == "1546")<a href="{{route('downloadPaymentReceiptPdf',$demand->unique_payment_id)}}">  
                                   <button class="btn btn-primary btn-sm" type="button">Receipt</button></a>
                                   @else 
                                   N/A
                                   @endif
    ...
    location.reload(); // modal & JS restore ke liye
	 ...
	 //$('[data-toggle="tooltip"]').tooltip();	 
	  $('.viewdemanddetails').click(function(){
	        ...
	        $.ajax({
	            url: "{{ route('get.olddemandbreakup') }}",
	            type: "GET",
