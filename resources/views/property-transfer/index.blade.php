        ...
        <div class="card-body">
            <div class="row">
                <div class="col-lg-12 mb-2">
                    @include('include.parts.property-selector', ['withOutLeaseHold' => $withOutLeaseHold])
                </div>
            </div>
                ...
                <div class="col-lg-12 col-12" id="propertyDetailsDiv">
                    <div class="row mb-3">
                        <!-- added by Swati on 07012026 -->
                            <div class="col-12">
                                <div class="alert alert-warning d-none" id="splitSearchMsg"></div>
                            </div>
                                {{-- Current Section --}}
                        <div class="col-lg-3">
                            <label for="current_section" class="form-label">Current Section</label>
                            <input type="text" class="form-control" id="current_section" readonly>
                        </div>
                        <div class="col-lg-3">
                            
                            <label for="section" class="form-label">Transfer to Section</label>
                        ...
                        <!-- added by swati 08012026 -->
                        <div class="col-lg-3">
                            <label for="propertyTransferDocument" class="form-label">Property Transfer Document</label>
                            <input type="file" class="form-control" id="propertyTransferDocument"
                                name="propertyTransferDocument" accept=".pdf,.jpg,.jpeg,.png">
                            <small class="text-muted">Optional (PDF/JPG/PNG)</small>
                        </div>
                        <!-- added by swati 08012026 -->
    ...
    <script>
        ...
        function getPropertyBasicDetail(propId, withOutLeaseHold) {
            $.ajax({
                ...
                success: function(response) {
                    // if (response.status == "success") displayPropertyDetails(response.data);//added by Swati on 07/01/2026
                    if (response.status == "success") displayPropertyDetails(response.data, response.meta);//added by Swati 07092026
                    else {
        ...
        function displayPropertyDetails(data, meta = null) {
            ...
            if (Array.isArray(data)) {
                ...
                // IMPORTANT: when data is array, don't set selectedOldPropertyId from array object
                $("#selectedOldPropertyId").val("");

                // added by swati on 08012026 : clear current section when multiple records
                $("#current_section").val("");
                // added by swati on 08012026 : clear current section when multiple records
            } else {
        ...
        $(document).ready(function() {
            ...
            $('#propertyTransferModalBtn').click(function() {
                //added by Swati on 07012026
                oldPropertyId = ($("#isSplittedSearch").val() == "1")
                    ? $("#masterOldPropertyId").val()
                    : ($("#selectedOldPropertyId").val() || $("#oldPropertyId").val());
            ...
            $('#propertyTransferConfirmBtn').click(function() {
                //added by Swati on 07/01/2026
                oldPropertyId = ($("#isSplittedSearch").val() == "1")
                    ? $("#masterOldPropertyId").val()
                    : ($("#selectedOldPropertyId").val() || $("#oldPropertyId").val());//added by Swati on 07/01/2026
                sectionId = $('#section').val();
                ...
                formData.append('oldPropertyId', oldPropertyId);
