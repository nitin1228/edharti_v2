...
<div class="card">
      ...
      <div class="d-none" id="detail-card">
        <div class="pb-3">
          ...
          <div class=""> <!-- this div add by anil on 21-01-2025-->
            ...
            <!-- In your property details section, add this container for flat details -->
            <div id="flat-detail-container" class="mt-4"></div>
            <!-- </table> -->
          </div>
        </div>
      ...
      <div class="{{ $propertyAllreadySelected ? '':'d-none' }}" id="input-form-container">
        <form id="demand-input-form" method="post" action="">
          ...
          <div class="row my-3">
           ...
           <div  class="row my-3">
            <div class="col-lg-12">
                <div class="calculation-info py-4">
                    <h6> The demand will be sent to the details below.</h6>
                    <div class="row">
                        <div class="col">
  ...
  <script>
    ...
    let differentialConversionCharges;
    let differentialconversionRemission;
    let differentialconversionSurcharge;
   ...
   @if (isset($applicationData))
    ...
    function appendPropertyDetail(row, isMultiple = false, rowNum = null) {
      ...
      let detailHTML = `
        ...
        <div class="part-details">
            ...
            if (row.flatDetails && Object.keys(row.flatDetails).length > 0) {
                $('#flatDiv').show();
                populateFlatDropdown(row.flatDetails);
            }
    ...
    //#Start Flat Details Code by Lalit on 23-12-2025
    // Function to populate flat dropdown
    function populateFlatDropdown(flatItems) {
        const flatSelect = $('#flat');

        // Clear existing options except the default "Select" option
        flatSelect.find('option:not(:first)').remove();

        // Check if we have valid flat items
        if (flatItems && Array.isArray(flatItems)) {
            // Loop through each flat item
            flatItems.forEach(flat => {
                // IMPORTANT: Your flat objects are Eloquent models, not plain objects
                // They have properties directly accessible, not under .attributes
                const flatNumber = flat.flat_number || `Flat ${flat.id}`;
                const flatId = flat.id || '';

                // Create option with id as value and flat_number as text
                const option = $('<option>', {
                    value: flatId, // id as value
                    text: flatNumber // flat_number as text
                });

                flatSelect.append(option);
            });

            // If using Bootstrap Selectpicker, refresh it
            if ($.fn.selectpicker) {
                flatSelect.selectpicker('refresh');
            }

            console.log(`Populated ${flatItems.length} flats in dropdown`);
        } else {
            console.error('No valid flat items found', flatItems);
        }
    }

    /** Get flat details when flat is selected */
    function getFlatDetails(flatId) {
        if (!flatId) {
            $("#flat-detail-container").empty();
            return;
        }
        ...
        $.ajax({
            ...
            beforeSend: function() {
                ...
                $("#flat-detail-container").html(
                    '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading flat details...</div>'
                );
            },
            success: function(response) {
                if (response.status === "success") {
                    // Format the flat data and append it to the flat details section
                    const flatDetailsHtml = formatFlatDetails(response.data);
                    $('#selectedFlatIdValue').val(response.data.id); // Set the hidden input value for selected flat ID
                    $("#flat-detail-container").html(flatDetailsHtml);
                } else {
                    $("#flat-detail-container").html(
                        '<div class="alert alert-danger">Failed to load flat details: ' + response
                        .message + '</div>');
                    console.error('Failed to fetch flat details:', response.message);
                }
            },
            error: function(xhr, status, error) {
                $("#flat-detail-container").html(
                    '<div class="alert alert-danger">Error loading flat details. Please try again.</div>'
                );
                console.error('AJAX error fetching flat details:', error);
            }
        });
    }

    /** Format flat details HTML */
    function formatFlatDetails(flat) {
        // Determine flat status
        let flatStatus = 'N/A';
        if (flat.property_flat_status == 951) {
            flatStatus = 'Lease Hold';
        } else if (flat.property_flat_status == 952) {
            flatStatus = 'Free Hold';
        }

        // Format purchase date if exists
        let purchaseDate = 'N/A';
        if (flat.purchase_date) {
            purchaseDate = flat.purchase_date.split('-').reverse().join('-');
        }
        $('#selectedFlatId').val(flat.id) //Added by Nitin to send flat value in request
        return `
    <div class="part-title mt-4">
        <h5>Selected Flat Details</h5>
    </div>
    <div class="part-details">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 col-12">
                    <table class="table table-bordered property-table-info">
                        <tbody>
                            <tr>
                                <th>Flat Number:</th>
                                <td>${flat.flat_number || 'N/A'}</td>
                                <th>Unique Flat ID:</th>
                                <td>${flat.unique_flat_id || 'N/A'}</td>
                                <th>Floor:</th>
                                <td>${flat.floor || 'N/A'}</td>
                            </tr>
                            <tr>
                                <th>File Number:</th>
                                <td colspan="2">${flat.unique_file_no || 'N/A'}</td>
                                <th>Flat Status:</th>
                                <td colspan="2">${flatStatus}</td>
                            </tr>
                            <tr>
                                <th>Flat Area:</th>
                                <td colspan="2">${flat.area_in_sqm || flat.area || 'N/A'} Sq. Mtr.</td>
                                <th>Rate per Sq. Mtr:</th>
                                <td colspan="2">₹${flat.rate ? customNumFormat(flat.rate) : 'N/A'}</td>
                            </tr>
                            <tr>
                                <th>Flat Value:</th>
                                <td colspan="2">₹${flat.value ? customNumFormat(flat.value) : 'N/A'}</td>
                                <th>Purchase Date:</th>
                                <td colspan="2">${purchaseDate}</td>
                            </tr>
                            <tr>
                                <th>Original Buyer:</th>
                                <td colspan="2">${flat.original_buyer_name || 'N/A'}</td>
                                <th>Present Occupant:</th>
                                <td colspan="2">${flat.present_occupant_name || 'N/A'}</td>
                            </tr>
                            <tr>
                                <th>Builder/Developer:</th>
                                <td colspan="3">${flat.builder_developer_name || 'N/A'}</td>
                                <th>Block/Plot:</th>
                                <td>${flat.block || 'N/A'}/${flat.plot || 'N/A'}</td>
                            </tr>
                            <tr>
                                <th>Known As:</th>
                                <td colspan="5">${flat.known_as || 'N/A'}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    // Add change event listener to flat dropdown
    $(document).ready(function() {
        // Listen for flat selection change
        $('#flat').on('change', function() {
            const selectedFlatId = $(this).val();
            $('#selectedFlatId').val(selectedFlatId);
            getFlatDetails(selectedFlatId);
        });

        // Also handle Bootstrap Selectpicker change if used
        $(document).on('changed.bs.select', '#flat', function() {
            const selectedFlatId = $(this).val();
            $('#selectedFlatId').val(selectedFlatId);
            getFlatDetails(selectedFlatId);
        });
    });
    //#End Flat Details Code by Lalit on 23-12-2025
          ...
          success: function(response) {
            if (response.status) {
              ...
              if (response.data.applicationData && response.data.applicationData.length > 0) {
               ...
               if(response.data.propertyContactDetails){
                    $('#additionalInputTo').val(response.data.propertyContactDetails.lessees_name ?? '');
                    $('#additionalInputAddress').val(response.data.propertyContactDetails.address ?? '');
                    $('#additionalInputEmail').val(response.data.propertyContactDetails.email ?? '');
                    $('#additionalInputPhone').val(response.data.propertyContactDetails.phone_no ?? '');
                    $('#additionalInputcontactRecordId').val(response.data.propertyContactDetails.contact_record_id ?? '');
                }
  ...
  function getAndAppendDemandInputs(selectedVal) {
    ...
    function appendUserInputs(checkbox) {
      ...
      switch (selectedSubhead) {
        ...
        case "DEM_CONV_CHG":
          ...
          break;
        case "DEM_DEF_CONV":
          appendDifferentialConversionInput(targetElement);
          break;
        case "DEM_LUC_RC":
    ...
    function appendUnearnedIncreaseInput(targetElement, inputType) {
        ...
        if (inputType == 2) {
            ...
            <div class="input-block">
                <label class="form-label">Last transaction value</label>
                <input type="number" min="0" class="form-control" id="unearned_increase_ltv" name="unearned_increase_ltv">
                <div class="error" id="unearned_increase_ltv_error"></div>
            </div>`;
            targetElement.append(html);
        }
    ...
    function appendDifferentialConversionInput(targetElement) {
        let propertyId = $("#selectedOldPropertyId").val();
        let dateInput = `<div class="input-block">
          <label class="form-label">Crucial Date</label>
            <input type="date" class="form-control" id="def_conv_crucial_date" name="def_conv_crucial_date" onchange="handleCrucialDateChange(this)">
            <div class="error" id="def_conv_crucial_date_error"></div>
          </div>`;
          targetElement.append(dateInput);
    }

    function handleCrucialDateChange(dateInput) {
        const dateString = $(dateInput).val();
        if (!dateString) return;

        // YYYY-MM-DD → year is index 0
        const yearVal = parseInt(dateString.split('-')[0], 10);
        if (yearVal < 1900 || yearVal > 3000) return;

        const propertyId = $("#selectedOldPropertyId").val();
        ...
        $.ajax({
            ...
            success: response => {
                const landRate = parseFloat(response.landRate);
                const propertyArea = parseFloat(response.propertyArea);

                const defConversionCharges   = parseFloat(response.charges.replaceAll(',',''));
                const defConversionRemission = parseInt(response.remission.replaceAll(',',''));
                const defConversionSurcharge = parseInt(response.surcharge.replaceAll(',',''));

                const defLandValue = landRate * propertyArea;
                const formula = response.formula;

                // Remove previous differential block (prevents duplicates)
                targetElement.find('.def-conversion-block').remove();

                const html = `
                <div class="col-lg-12 def-conversion-block">
                    <div class="calculation-info">
                        &diams; <b>Land Value →</b> ₹${customNumFormat(Math.round(defLandValue*100)/100)}<br>
                        &diams; <b>Land Rate →</b> ₹${customNumFormat(Math.round(landRate*100)/100)}<br>
                        &diams; <b>Plot Area →</b> ${customNumFormat(Math.round(propertyArea*100)/100)} Sqm.<br>
                        &diams; <b>Total conversion charges</b> [${formula}] → <b>₹${customNumFormat(Math.round(defConversionCharges*100)/100)}</b><br>
                        &diams; <b>Applicable remission →</b> ₹${customNumFormat(Math.round(defConversionRemission*100)/100)}<br>
                        &diams; <b>Applicable surcharge →</b> ₹${customNumFormat(Math.round(defConversionSurcharge*100)/100)}
                    </div>
                </div>
                ...
                <div class="col-lg-12 def-conversion-block">
                    <div class="form-check">
                        ...
                        <input type="hidden" name="def_conv_land_value" value="${defLandValue}">
                        <input type="hidden" name="def_conv_land_rate" value="${landRate}">
                        <input type="hidden" name="def_conv_plot_area" value="${propertyArea}">
                        <input type="hidden" name="def_conv_remission_amount" id="def_conv_remission_amount" value="${defConversionRemission}">
                        <input type="hidden" name="def_conv_surcharge_amount" id="def_conv_surcharge_amount" value="${defConversionSurcharge}">
                        <input type="hidden" name="def_conv_formula" value="${formula}">
                        <input type="hidden" name="def_conv_charges" id="def_conv_charges" value="${defConversionCharges}">

                        <div class="row mt-2">
                            <div class="col-lg-3">
                                <input class="form-check-input" type="checkbox" name="def_conv_remission">
                                <label class="form-check-label">Allow Remission</label>
                            </div>

                            <div class="col-lg-3">
                                <input class="form-check-input" type="checkbox" name="def_conv_surcharge">
                                <label class="form-check-label">Add Surcharge</label>
                            </div>

                            <div class="col-lg-3">
                                <label class="form-label">Already Paid Conversion Charges</label>
                                <input class="form-control" type="number" name="def_conv_paid" id="def_conv_paid">
                                <div class="error" id="def_conv_paid_error"></div>
                            </div>
                        </div>
    ...
    function appendEnchroachmentInput(targetElement) {

    // Remove + button from previous rows
    targetElement.find('.add-encroachment').remove();
    ...
    */
    ...
    if (targetElement.find('.encroachment-formula-card').length === 0) {
    ...
    /*
    |--------------------------------------------------------------------------
    | Encroachment Repeater Card
    |--------------------------------------------------------------------------
    */

    let html = `

        <div class="row encroachment-row border rounded p-2 pb-3 m-1"
...
$(document).on('click', '.add-encroachment', function () {
...
$(document).on('click', '.remove-encroachment', function () {
    ...
    let lastRow = container.find('.encroachment-row').last();

    if (
        lastRow.length &&
        lastRow.find('.add-encroachment').length === 0
    ) {
    ...
    */
    ...
    function appendStandatdPenaltyInput(targetElement) {
      let html = `<div class="input-block">
        ...
        <div class="input-block">
            <label class="form-label">Description</label>
            <textarea class="form-control" name="standard_penalty_description" id="standard_penalty_description" rows="2" placeholder="Add description of penalty (min. 50 characters)"></textarea>
            <div class="error" id="standard_penalty_description_error"></div>
        </div>
    </div>
        `;
    ...
    //for setelling the already paid amount of applicant START ************
     function appendSettledAmountInput() {
      let html = `<div class="demand-item-container settled-demand-input" style="
    background: #d2ffd2;
">
      <div class="col-lg-12 my-1">
        <div class="form-check">
          <h6>Amount Already Paid</h6>
          <input type="hidden" name="demand_amount[DEM_SETTLED_AMOUNT][${$(document).find('.settled-demand-input').length}]" id="include-demand-amount">
        </div>
      </div>
      <div class="col-lg-12 user-inputs" id="user-inputs">
        <div class="input-block">
          <label for="" class="form-label">Head</label>
          <input type="text" name="settled_title[${$(document).find('.settled-demand-input').length}]" value="Already Paid Amount" id="settled_title" class="form-control">
          <div class="error" id="settled_title_error"></div>
        </div>
        <div class="input-block">
          <label class="form-label">Amount</label>
          <input type="number" min="0" name="settled_amount[${$(document).find('.settled-demand-input').length}]" id="settled_amount" class="form-control" step="0.01">
          <div class="error" id="settled_amount_error"></div>
        </div>
        <div class="input-block">
          <label for="" class="form-label">Paid On</label>
          <input type="date" name="settled_date_from[${$(document).find('.settled-demand-input').length}]" id="settled_date_from" class="form-control">
          <div class="error" id="settled_date_from_error"></div>
        </div>
        <div class="col-lg-12 d-flex mt-2 justify-content-between">
          <button type="button" class="btn btn-sm btn-primary btn-calculate me-auto" data-calculation-type="DEM_SETTLED_AMOUNT">Add</button>
    ...
    //for setelling the already paid amount of applicant END ************
    ...
    function appendManualInput() {
      ...
      <div class="col-lg-12 user-inputs" id="user-inputs">
        ...
        <div class="col-lg-12 mt-2">
          <label class="form-label">Description</label>
          <textarea class="form-control" name="manual_description[${$(document).find('.manual-demand-input').length}]" id="manual_description" rows="2" placeholder="Add description of demand (min. 50 characters)"></textarea>
          <div class="error" id="manual_description_error"></div>
        </div>
        <div class="col-lg-12 d-flex mt-2 justify-content-between">
          <button type="button" class="btn btn-sm btn-primary btn-calculate me-auto">Add</button>
    ...
    function appendOthersInput(targetElement) {
      let html = `<div class="input-block">
        ...
        <div class="input-block">
            <label class="form-label">Description</label>
            <textarea class="form-control" name="others_description" id="others_description" rows="2" placeholder="Add description of penalty (min. 50 characters)"></textarea>
            <div class="error" id="others_description_error"></div>
        </div>
        `;
      targetElement.append(html);
    ...
    $(document).on('click', '.btn-calculate', function(ev) {
      ...
      let calculatingFor;
      let calculationType = $(this).data('calculation-type');
      if (calculationType === 'DEM_SETTLED_AMOUNT') {
        calculatingFor = "DEM_SETTLED_AMOUNT";
      } else {
          calculatingFor = (checkbox.length > 0) ? checkbox.attr('name') : "DEM_MANUAL";
      }
      ...
      switch (calculatingFor) {
        ...
        case "DEM_CONV_CHG":
          ...
          break;
        case "DEM_DEF_CONV":
          calculateDifferntialConversionInput(inputElements);
          break;
        case "DEM_LUC_RC":
  ...
  function calculateGroundRent(inputElements) {
    ...
    function calculateUnearnedIncrease(inputElements) {
        ...
        let ltvInput = inputElements.find('#unearned_increase_ltv');
        let ltv = 0;
        ...
        if (ltvInput.length > 0) {
            ltv = parseFloat(ltvInput.val()) || 0;
            if (isNaN(ltv)) {
                    $('#unearned_increase_ltv_error').text("Last transaction value is required.")
                }
        }
        ...
        let isError = landValue == 0 || (isTransferComplete && considerationValue == 0)|| (ltv && ltv==0)
        if (!isError) {
            let calculationValue = Math.max(landValue, considerationValue) //callculate and round off to 2 decimal points
            let unearnedIncreaseValue = (calculationValue - ltv)/2;
            unearnedIncreaseValue = Math.max(0, unearnedIncreaseValue);
            unerarnedIncrease = Math.round(unearnedIncreaseValue * 100)/100;
            let result =
                `Unearned Increase (${landValue >= considerationValue ? 'Land Value':'Consideration Value'}- Last Transaction Value)/2  = ₹ ${customNumFormat(unerarnedIncrease)} [Calculated on land value or consideration value whichever is greater]`;
            displayDemandCalculationResult(inputElements, result)
    ...
    /* function calculateConversionCharges(inputElements) {
      ...
      function calculateDifferntialConversionInput(inputElements) {
        /* let landValueInput = inputElements.find('#conversion_land_value');*/
        let allowRemssionCheck = inputElements.find('input[name="def_conv_remission"');
        let allowSurchargeCheck = inputElements.find('input[name="def_conv_surcharge"');
        ...
        let allowRemission = allowRemssionCheck && allowRemssionCheck.is(':checked');
        let allowSurcharge = allowSurchargeCheck && allowSurchargeCheck.is(':checked');
        differentialConversionCharges = differentialConversionCharges ?? $('#def_conv_charges').val();
        differentialconversionRemission = differentialconversionRemission ?? $('#def_conv_remission_amount').val();
        differentialconversionSurcharge = differentialconversionSurcharge ?? $('#def_conv_surcharge_amount').val();
        //let conversionCharges = landValue * (20 / 100); //callculate and round off to 2 decimal points
        // let netConversion = allowRemission ? conversionCharges - conversionRemission : conversionCharges;
        let netConversion = +differentialConversionCharges;
        if (allowRemission) {
            netConversion -= +differentialconversionRemission;
        }
        if (allowSurcharge) {
            netConversion += +differentialconversionSurcharge;
        }

        netConversion = Math.round(netConversion * 100) / 100;
        let paidConversionInput = $('#def_conv_paid');
        console.log(paidConversionInput);
        let paidConversionAmount= +paidConversionInput.val();
        console.log(paidConversionAmount);
        let differntialConversionCharges = Math.max((netConversion - paidConversionAmount),0)

        differntialConversionCharges = Math.round(differntialConversionCharges)
        let result =
            `Payable differential conversion charges [net conversion charges on crucial date - already paid conversion charges] = ₹ ${customNumFormat(differntialConversionCharges)} (rounded off)`
        displayDemandCalculationResult(inputElements, result)
        fillDemandAmount(inputElements, differntialConversionCharges);
    }
    ...
    function calculateSettledAmount(inputElements){
      inputElements.find('.error').empty();
      let inputError = false;
      let amountInput = inputElements.find('#settled_amount');
      let amount = 0;
      if (amountInput.length > 0) {
        amount = parseFloat(amountInput.val()) || 0;
        if (amount <= 0) {
          inputElements.find('#settled_amount_error').text('Amount is required')
          inputError = true;
        }
      } else {
        inputError = true;
      ...
      let titleInput = inputElements.find('#settled_title');
      if (titleInput.length > 0) {
        if (titleInput.val() == "") {
          inputElements.find('#settled_title_error').text('Title is required')
          inputError = true;
        }
      } else {
        inputError = true;
      ...
      let settledDateInput = inputElements.find('#settled_date_from');

      let settledDateVal = settledDateInput.val();

      // Only validate if at least one date is filled
      if (settledDateVal) {
        let settledDate = new Date(settledDateVal);
        let today = new Date();

        // If "from" date is filled but invalid
        if (isNaN(settledDate.getTime())) {
          settledDateInput.siblings('.error').text('Valid date is required.');
          inputError = true;
        }
        if(!isNaN(settledDate.getTime()) && settledDate > today)
        {
          settledDateInput.siblings('.error').text('Payment date can not be later than today');
          inputError = true;
        }
      }
      else{
      ...
      if (!inputError) {
        let result = `Demand head for amount &#8377;${customNumFormat((Math.round(amount*100)/100).toFixed(2))} added successfully.`;
        displayDemandCalculationResult(inputElements, result)
        fillDemandAmount(inputElements, amount,true);
      }
    ...
    function fillDemandAmount(target, amount,isDeduction = false) {
      let inputElement = target.parent().find('#include-demand-amount');
       if (inputElement.length > 0) {
        inputElement.val(isDeduction ? -amount : amount);
      }
      calculateTotalAmount();
    }
